...
-- Site-specific, non-secret infrastructure (Coin/OpenNebula/log hosts, the QNX
-- guest address, default platform config) lives in coin.ini next to this script
-- instead of being baked into it. The file is not committed; environment
-- variables still override it.
local INI_SAMPLE = [[
[coin]
base_url = https://coin.example.com/coin
log_host = https://testresults.example.com
one_xmlrpc = http://opennebula.example.com:2633/RPC2
platform_config = qnx-710-x86_64-developer-build-on-linux
vm_user = qt
qnx_ip = 172.31.1.10
qnx_user = root
]]
...
local function iniPath()
    return PluginSpec.pluginDirectory:resolvePath("coin.ini")
end

local infraConf   -- key -> value, loaded lazily from coin.ini
local infraError  -- set when coin.ini is missing; carries the how-to text

-- Parse a trivial INI ("key = value", ";"/"#" comments and [section] ignored).
local function loadInfra()
    infraConf = {}
    local f = io.open(iniPath():nativePath(), "r")
    if not f then
        infraError = "Coin: infrastructure file not found:\n    "
            .. iniPath():toUserOutput()
            .. "\n\nCreate it (adjust the values to your site):\n\n" .. INI_SAMPLE
        return
    end
    for line in f:lines() do
        local key, val = line:match("^%s*([%w_]+)%s*=%s*(.-)%s*$")
        if key then infraConf[key] = val end
    end
    f:close()
end

-- Resolve a site setting: environment variable wins, then coin.ini. Returns ""
-- when neither provides it (infraError then explains how to create coin.ini).
local function infra(key, envName)
    if not infraConf then loadInfra() end
    local v = envName and os.getenv(envName)
    if v and v ~= "" then return v end
    return infraConf[key] or ""
end
...
local function waitForVm(taskId, exclude, timeoutS)
    local deadline = os.time() + (timeoutS or 1800)
    while os.time() < deadline do
        local ok, _, rip = pcall(resolveDebugVm, exclude)
...
resolveDebugVm = function(exclude)
    local best
    for _, vm in ipairs(myVms()) do
        local n = (vm.name or ""):lower()
        if vm.id and n:find("debugvm", 1, true)
                and not (exclude and exclude[vm.id]) then
...
-- Ids of my current debug VMs, captured before scheduling so waitForVm can tell
-- a freshly created VM from a pre-existing (already reachable) one.
local function debugVmIds()
    local ids = {}
    for _, vm in ipairs(myVms()) do
        if vm.id and (vm.name or ""):lower():find("debugvm", 1, true) then
            ids[vm.id] = true
        end
    end
    return ids
...
local function bringUp(configName)
    ...
    -- Snapshot the debug VMs I already have, so waitForVm gates on the NEW one
    -- and does not latch onto a leftover (already reachable) VM immediately.
    local existing = debugVmIds()
    ...
    local ip = waitForVm(taskId, existing)
    if not ip then
...
local function setupAspect()
    ...
    Settings.baseUrl = S.StringAspect.create({
        ...
        defaultValue = infra("base_url", "COIN_BASE"),
    })
    Settings.config = S.StringAspect.create({
        ...
        defaultValue = infra("platform_config", "COIN_CONFIG"),
    })
    Settings.logHost = S.StringAspect.create({
        ...
        defaultValue = infra("log_host", "COIN_LOG_HOST"),
    })
    Settings.vmUser = S.StringAspect.create({
        ...
        defaultValue = infra("vm_user", "COIN_VM_USER"),
    })
    ...
    Settings.oneXmlrpc = S.StringAspect.create({
        ...
        defaultValue = infra("one_xmlrpc", "ONE_XMLRPC"),
    })
...
local function repro30629()
    ...
    local qnxIp = infra("qnx_ip", "COIN_QNX_IP")
    local qnxUser = infra("qnx_user", "COIN_QNX_USER")
...
local function setupWorkspaceMode()
    ...
    local status = Label {
        text = infraError and "coin.ini missing - see General Messages for the format."
            or 'Use "List configurations" or "Fetch" to begin.',
    }
    local vmRows, vmVisible = {}, {}
    local configNames, configVisible = {}, {}
    local vmFilterText, configFilterText = "", ""
    local fetchButton
    local vmFilter, configFilter
    local configSpinner
...
Tools > Options > Coin.
    ...
    local function matches(hay, needle)
        return needle == "" or string.find(hay:lower(), needle:lower(), 1, true) ~= nil
    end

    -- Re-populate a list from its backing rows, keeping only the ones matching
    -- the filter. The *Visible arrays map a shown row back to its backing entry
    -- so the buttons act on the right item regardless of the filter.
    local function applyVmFilter()
        vmVisible = {}
        local items = {}
        for _, vm in ipairs(vmRows) do
            local label = string.format("%-8s  %s", vm.id or "?", vm.name or "?")
            if matches(label, vmFilterText) then
                vmVisible[#vmVisible + 1] = vm
                items[#items + 1] = label
            end
        end
        list:setItems(items)
    ...
    local function applyConfigFilter()
        configVisible = {}
        local items = {}
        for _, name in ipairs(configNames) do
            if matches(name, configFilterText) then
                configVisible[#configVisible + 1] = name
                items[#items + 1] = name
            end
        end
    ...
    local function refresh()
        if infraError then error(infraError) end
    ...
    local function terminateSelected()
        ...
        local vm = row >= 0 and vmVisible[row + 1] or nil
        if not vm then
    ...
    local function bringUpSelected()
        ...
        local cfg = row >= 0 and configVisible[row + 1] or nil
        bringUp(cfg)
        refresh()
    end
    ...
    vmFilter = LineEdit {
        placeholderText = "Filter VMs",
        onTextChanged = function(text) vmFilterText = text; applyVmFilter() end,
    }
    configFilter = LineEdit {
        placeholderText = "Filter configurations",
        onTextChanged = function(text) configFilterText = text; applyConfigFilter() end,
    }
    configSpinner = SpinnerOverlay(configList)

    coinMode = Mode.create {
        id = "Coin.Mode",
        displayName = "Coin",
        priority = 10,
        icon = PluginSpec.pluginDirectory:resolvePath("coin.svg"),
        widget = Widget {
            Column {
                -- The two panes carry the normal workflow left to right: pick a
                -- configuration and bring up a VM on the left, then work with the
                -- running VMs on the right. Each pane has a filter and its own
                -- actions; the reference text sits in the small bottom pane.
                Splitter {
                    orientation = "vertical",
                    stretchFactors = { 4, 1 },
                    -- A QSplitter takes widgets, not layouts, so wrap the row of
                    -- panes (itself a layout) in a Widget.
                    Widget { Row {
                        Widget { Column {
                            Label { text = "1. Configurations" },
                            configFilter,
                            configList,
                            Row {
                                PushButton {

...
Dialog {
    ...
    contentItem: Column {
        ...
        Row {
            ...
            HelperWidgets.Button {
                ...
                onClicked: {
                    const path = AssetLibraryBackend.rootView.getUniqueEffectPath(root.dirPath, effectName.text)
                    if (AssetLibraryBackend.rootView.createNewEffect(path))
                        root.accept()
                    else
    ...
    onOpened: {
        const path = ItemLibraryBackend.rootView.getUniqueEffectPath(root.dirPath, "Effect01")

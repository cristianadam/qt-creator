...
# BridgeEngine. See bridge-protocol-design.md.
#
...
class DapServer():
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                self.dumper.warn('DAP read error: %s' % error)
                break
            if message is None:
                break
    ...
    def _dispatch(self, request):
        ...
        try:
            ...
            self.dumper.warn('DAP handler %s failed: %s' % (command, error))
    ...
    def _execute(self, command):
        ...
        try:
            ...
            self.dumper.warn('DAP execute %r failed: %s' % (command, error))

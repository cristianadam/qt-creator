...
class Dumper(DumperBase):
    def __init__(self):
        ...
        # asking that module first is one GetTypeId() instead of a search.
    ...
    def vtable_owner(self, vtable: int):
        ...
        # subobject the table serves within the complete object; ('', 0) where
        # there is no vtable, None where the table cannot be told. The engine
        # names the nearest symbol and undecorates the tables a class has for
        # each of its bases to the same name, so the symbol has to sit at the
        # address itself, and which subobject the table serves is read off its
        # RTTI locator.
    ...
    def vtable_subobject_offset(self, vtable: int):
        ...
        # has none, so what the slot leads to is checked for the shape. A
        # constructor displacement means a virtual base, whose offset takes
        # more than this to find.
        ptr_size = self.ptrSize()
        locator = self.extract_pointer_at_address(vtable - ptr_size)
        if not self.couldBePointer(locator):
            return None
        if ptr_size == 8:

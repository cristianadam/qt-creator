...
#               pass
    ...
    def runDapServer(self):
        ...
        # DAP-shaped server, which owns stdio from here on. See bridge.py.
        from bridge import DapServer
        DapServer(self).run()
...
class ObjectAvailableBreakpoint(gdb.Breakpoint):
    ...
    def stop(self):
        # An internal breakpoint has no number the engine could recognize the
        # stop by, so name it before the stop record arrives.
        print('interpreteravailable={state="reached"}')
        return True

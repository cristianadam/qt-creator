...
class Dumper(DumperBase):
    ...
    def markPendingInterrupt(self, args):
        # Interrupt delivered externally (real SIGINT), not via Stop().
        self.isInterrupting_ = True
        self.reportResult('', args)
    ...
    def resetInferior(self, args):
        # Kill-and-relaunch (plain local launch only). Not calling
        # runEngine() again avoids a second loop() thread.
    ...
    def fetchSections(self, args):
        ...
        # Top-level sections are PT_LOAD segments; real ELF sections are sub-sections.
        entries = []

...
class Dumper(DumperBase):
    ...
    def waitForNativeStop(self):
        # Synchronously pump the listener until the inferior stops (or
        # exits) again. Used to drive a multi-step descent without
        # surfacing the intermediate run/stop cycles to the engine.
        event = lldb.SBEvent()
        listener = self.debugger.GetListener()
        for _ in range(600):
            ...
            state = lldb.SBProcess.GetStateFromEvent(event)
            self.eventState = state
            if state == lldb.eStateStopped or state == lldb.eStateExited:
                return state

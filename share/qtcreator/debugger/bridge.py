...
class DapServer():
    ...
    def _takeOverStopEvents(self):
        ...
        try:
            import gdbbridge
            gdb.events.stop.disconnect(gdbbridge.interpreterStopHandler)
    ...
    def _decodedLocations(self, bp):
        ...
        for sal in sals:
            ...
            decoded.append(types.SimpleNamespace(
                address=int(sal.pc), function=None, source=source,
                fullname=sal.symtab.fullname() if sal.symtab is not None else None,
                enabled=bp.enabled))
        return decoded

    def _resultToMi(self, value):
        # Like the dumper's resultToMi(), but escaping the strings. A Windows
        # path would otherwise leave a stray escape sequence in the payload the
        # C++ GdbMi reader parses ("C:\\Users" -> \\U).
        if isinstance(value, bool):
            return '"%d"' % int(value)
        if isinstance(value, dict):
            return '{' + ','.join('%s=%s' % (k, self._resultToMi(v))
                                  for k, v in value.items()) + '}'
        if isinstance(value, list):
            return '[' + ','.join(self._resultToMi(v) for v in value) + ']'
    ...
    def _breakpointToMi(self, bp):
        ...
        return self._resultToMi(result)

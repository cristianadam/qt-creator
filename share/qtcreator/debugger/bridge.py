...
class DapServer():
    ...
    def cmd_qtc_runStartupCommands(self, request):
        ...
        if script:
            self._execute_quietly('source %s' % script)
    ...
    def cmd_qtc_loadSymbols(self, request):
        args = request.get('arguments', {})
        if args.get('all'):
            self._execute_quietly('sharedlibrary .*')
            self.sendResponse(request)
            return
        ...
        if not module:
            ...
            return
        self._execute_quietly('sharedlibrary %s' % dotEscape(module))
        self.sendResponse(request)
    ...
    def cmd_qtc_configureTarget(self, request):
        ...
        for mapping in args.get('sourcePathMap', []):
            source = gdbLineArgument(mapping.get('from', ''), 'source path')
            target = gdbLineArgument(mapping.get('to', ''), 'source path')
            if source is not None and target is not None:
                self._execute_quietly('set substitute-path %s %s' % (source, target))

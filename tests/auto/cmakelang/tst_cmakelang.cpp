...
void tst_CMakeLang::documentationOfCMakeHelp()
{
    ...
    const auto documentationOf = [&](const QString &path,
        ...
        options.resolveInclude =
            [&](const QString &type, const QString &included) -> std::optional<QString> {
            const std::optional<QString> source = read(directory.filePath(included));
    ...
    // A command of CMake itself.
    const Documentation addExecutable
        = documentationOf(root + "/command/add_executable.rst", "add_executable");
    QVERIFY(addExecutable.brief().startsWith("### add_executable"));
    QVERIFY(addExecutable.brief().contains(
    ...
    // A module, whose documentation stands in the module itself.
    const Documentation module = documentationOf(root + "/module/FindPackageMessage.rst",
                                                 "FindPackageMessage");
    QVERIFY(module.brief().contains("FindPackageMessage"));

    // The command that module provides, which the Help has no file of its
    // own for.
    const Documentation command = documentationOf(root + "/module/FindPackageMessage.rst",
                                                  "find_package_message");
    QCOMPARE(command.name, "find_package_message");
    QCOMPARE(command.signatures(),
             QStringList("find_package_message(<PackageName> <message> <details>)"));

    const QList<ArgumentDoc> arguments = command.arguments();
    QCOMPARE(arguments.size(), 3);
    QCOMPARE(arguments.at(0).name, "<PackageName>");
    QVERIFY(arguments.at(0).documentation.startsWith("The name of the package"));
    ...
    QVERIFY(modes.value("WRITE").startsWith("```cmake"));
    QVERIFY(modes.value("WRITE").contains("file(WRITE <filename> <content>...)"));
    QVERIFY(modes.value("WRITE").contains("Write `<content>` into a file"));

    // Two modes that share what is said about them each get to say it.
    QVERIFY(modes.value("APPEND").contains("file(APPEND <filename> <content>...)"));
    QVERIFY(modes.value("APPEND").contains("Write `<content>` into a file"));
    ...
    // A term that names several arguments names each of them: file()
    // documents "``PERMISSIONS`` and ``FILE_PERMISSIONS``" in one.
    // file() says what each of them is for in every mode that takes them,
    // so what is said of one adds up over the modes.
    QVERIFY(modes.contains("PERMISSIONS"));
    QVERIFY(modes.contains("FILE_PERMISSIONS"));
    QVERIFY(modes.value("FILE_PERMISSIONS").contains("overrides"));
    QVERIFY(modes.value("DIRECTORY_PERMISSIONS").contains("directories"));

...
private slots:
    ...
    void roundtripV2()
    {
        ...
        for (const TraceDirectory::Trace &trace : td.traces()) {
            ...
            {
                TraceWriter tw(schema, &metaFile);
                QVERIFY(!tw.hasError());
                ...
                for (int i = 0; i < originals.size(); ++i) {
                    ...
                    for (const EventRecord &e : originals[i].events) {
                        w->writeEvent(e.eventClassId, e.payload,
                                      e.specificContext, e.commonContext,
                                      e.timestamp);
                    }
                    QVERIFY(!w->hasError());
                }
                tw.close(); // flushes streams
            }
            metaFile.close();

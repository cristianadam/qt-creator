...
// finalize(). The views can paint in that window, so they must not use those
// rows to index their per-row arrays.
...
void tst_TrackPainterProperties::rowsBeyondRowCount()
{
    ...
    TimelineOverviewWidget overview(&aggregator, &zoom);
    overview.resize(200, 50);
    QPixmap overviewTarget(overview.size());
    overview.render(&overviewTarget);
}

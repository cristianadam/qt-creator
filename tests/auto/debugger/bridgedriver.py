...
#
# Each check exits 0 on success, 1 with an explanation on failure, and prints
# "SKIP: <reason>" (exit 0) where the platform cannot run it.
...
class Peer():
    ...
    def raw(self, timeout=5.0):
        # Drain whatever is readable, bounded so a missing message fails the
        # check instead of hanging.
        while select.select([self.readFd], [], [], timeout)[0]:
            chunk = os.read(self.readFd, 65536)
            if not chunk:
                break
            self.buffer += chunk
            if not select.select([self.readFd], [], [], 0.1)[0]:
                break
        return self.buffer

    def messages(self, timeout=5.0):
        # Parse the Content-Length framing strictly: anything else in the
        # stream (a stray print, say) makes this fail.
        data = self.raw(timeout)
        messages = []
        while data:
            header, _, rest = data.partition(b"\r\n\r\n")
            if not _:
                raise AssertionError("unframed trailing bytes: %r" % data[:200])
...
def check_inferior_output_event(bridge):
    ...
    if bridge.termios is None:
        print("SKIP: no termios on this platform")
        return
    peer = Peer(bridge)

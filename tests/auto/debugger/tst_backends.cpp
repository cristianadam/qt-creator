...
// The tail of a captured wire log, for a failure message. Each line is elided
// first: setupInferior() carries the whole hex-encoded environment on one line,
// which on its own outruns a CI log's per-line budget and hid the rest of the
// dump the first time this was reported from macOS CI.
...
// Whether a *running* inferior can be interrupted at all on this host. Both
// exceptions are documented in full at stepsContinuesAndInterrupts(), which
// skips for the same two reasons: pdb interrupts through a Python SIGINT
// handler that Windows' DebugBreakProcess() never triggers, and cdb's ctrl-c
// stub broadcasts to the whole console, which would kill this console-
// application test runner itself.
...
void tst_backends::continueWhileRunningReportsRunFailed()
{
    ...
    // The check below is that a *wrong* event does not arrive, which needs
    // something later to wait for (see CLAUDE.md's testing rules). A backend
    // that answers the rejected request supplies that itself; one that does not
    // is interrupted below instead. Only where neither is possible - a running
    // inferior that cannot be stopped at all, see canInterruptRunningInferior()
    // - is there nothing here to bound the check on.
    ...
    if (testData.answersRedundantContinue) {
        ...
        // Nothing answers this request, so an interrupt of our own is the bound:
        // commands are handled in order, so the StopOk it reports cannot arrive
        // before the rejected request was processed - an InferiorIll would
        // already be here by then. (An explicitly requested interrupt is a
        // StopOk, not a SpontaneousStop - see m_interruptRequested in gdbimpl.h.)
        debuggerBackend->execute({ExecutionCommand::Interrupt});
        QTRY_VERIFY2_WITH_TIMEOUT(debuggerBackend->contains(InferiorEvent::StopOk),
    ...
    // Poking the inferior needs a backend that can act on a command while it
    // runs; the interrupt above has already stopped it on that path.
    if (canEndSpinLoop)
...
void tst_backends::insertsQmlBreakpointBeforeDumpersLoad()
{
    ...
    connect(engine, &DebuggerEngineInterface::breakpointModified, this,
            [&modifiedReports](const GdbMi &data) { modifiedReports.append(data); });
    // Same instrumentation as insertsQmlBreakpointAndStopsAtIt(): both tests fail on
    // this one assertion on macOS CI, and neither the resolver hook firing nor the
    // retry's own reply is visible from the event stream alone.
    QStringList wire;
    connect(engine, &DebuggerEngineInterface::message, this,
            [&wire](const QString &text, int, int) { wire.append(text); });

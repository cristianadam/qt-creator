...
// A gzip stream whose stored file name is "test-data.txt" and whose content is
// "This file is called test-data.txt\n".
...
private slots:
    ...
    void tst_gzip_decompress()
    {
        const Result<QByteArray> out
            = gzipDecompress(QByteArray((const char *) test_data_txt_gz, 66));
        QVERIFY(out);
        QCOMPARE(*out, "This file is called test-data.txt\n");

        const Result<QByteArray> notCompressed = gzipDecompress("plain bytes, not a stream");
        QVERIFY(!notCompressed);
    }

    // Note: this passes with or without the copy_data error propagation, because
    // archive_write_finish_entry catches the truncation too. It guards against
    // truncation becoming silent, not against that specific defect.
    void tst_truncated_archive_fails()
    {
        struct archive *a = archive_write_new();
        archive_write_add_filter_none(a);
        archive_write_set_format_pax_restricted(a);

        const FilePath archive = FilePath::fromString(tempDir.path() + "/truncated.tar");
        write_raw_archive(a, archive, {{"big.bin", QByteArray(64 * 1024, 'x'), AE_IFREG, 0644, {}}});

        // Keep the header and the first part of the content, drop the rest.
        const QByteArray full = *archive.fileContents();
        QVERIFY(archive.writeFileContents(full.left(4096)));

        Unarchiver unarchiver;
        unarchiver.setArchive(archive);
        unarchiver.setDestination(FilePath::fromString(tempDir.path() + "/dest-truncated"));
        unarchiver.start();
        QVERIFY2(!unarchiver.result(), "a truncated archive extracted without an error");
    }

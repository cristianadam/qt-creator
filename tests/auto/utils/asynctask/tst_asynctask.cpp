...
void tst_AsyncTask::mapReduce_data()
{
    ...
    const auto createTask = [=](const TaskBase &executeMode,
                                const SetupHandler &setupHandler,
                                const DoneHandler &doneHandler) {
        return Task {
            executeMode,
            OnSubTreeSetup(initTree),
            ...
            OnSubTreeDone(doneHandler)
        };
    };

    const Task parallelRoot = createTask(parallel, setupAsync, handleTreeParallel);
    const Task parallelRootWithFI = createTask(parallel, setupAsyncWithFI, handleTreeParallel);
    const Task parallelRootWithTP = createTask(parallel, setupAsyncWithTP, handleTreeParallel);
    const Task sequentialRoot = createTask(sequential, setupAsync, handleTreeSequential);
    const Task sequentialRootWithFI = createTask(sequential, setupAsyncWithFI, handleTreeSequential);
    const Task sequentialRootWithTP = createTask(sequential, setupAsyncWithTP, handleTreeSequential);
    ...
    const Task simpleRoot = {
        sequential,
        OnSubTreeSetup([] { s_sum = 0; }),
    ...
    const Task stringRoot = {
        parallel,
        OnSubTreeSetup([] { s_sum = 90.0; }),

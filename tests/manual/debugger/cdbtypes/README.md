...
`app` forward declares three structs and defines a pointer to each. `problib`
defines two of them, and `app` loads that library before it stops in
`afterLoad()`, so at the stop every type is available in some module. It needs
no Qt.

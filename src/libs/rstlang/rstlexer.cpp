...
{
    const QStringView content = line.content(_source);
    const QStringView markup = content.sliced(qMin<qsizetype>(3, content.size())).trimmed();
...
// What stands behind the marker of a list item or of a field opens the body
// of it, indented to the column it is written at.
void Lexer::openValue(const Line &line, int markerLength)
{
    const QStringView content = line.content(_source);
    ...
    const int column = line.indent + int(start);
    openIndent(column, line);

    Token &tk = push(Parser::T_TEXT, line.textStart + int(start), line.number, column + 1);
    tk.indent = column;
    tk.length = line.end - tk.position;
    tk.spelling = _source.mid(tk.position, tk.length);
    tk.text = tk.spelling;
    _blockEnd = line.end;
    _literalFollows = tk.text.endsWith("::"_L1);

...
{
    if (isCTF1Metadata(metadataDevice)) {
        auto tsdlResult = readCTF1TsdlText(metadataDevice);
        if (!tsdlResult) return Utils::ResultError(tsdlResult.error());
        auto schemaResult = TsdlParser::parse(*tsdlResult);
        if (!schemaResult) return Utils::ResultError(schemaResult.error());
...
DataStreamReader *TraceReader::openStream(const DataStreamClass &dsc, QIODevice *dataDevice)
{
    ...
    auto reader = std::make_unique<DataStreamReader>(dataDevice, dsc, tc, &m_schema);

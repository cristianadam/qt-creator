...
DataStreamWriter::DataStreamWriter(QIODevice *dev,
    ...
    : m_dev(dev), m_dsc(dsc), m_tc(tc),
      m_packetSizeLimitBytes(packetSizeLimitBytes), m_uuid(uuid)
{
...
void DataStreamWriter::writeEvent(quint64 eventClassId,
                                  ...
                                  quint64 timestamp)
{
    ...
    if (m_haveLastTimestamp && m_clockTimestampBits < 64) {
        ...
        if (timestamp >= m_lastTimestamp && (timestamp - m_lastTimestamp) >= window)
            rollForClock = true;
    }
    const bool rollForLengthField = m_oneEventPerPacket && m_packetHasEvent;
    if (m_packet->contentBits() / 8 >= m_packetSizeLimitBytes
        || rollForClock || rollForLengthField) {
        m_packet->finalize();
...
bool DataStreamWriter::flush()
{
    if (!m_packetOpen)
        return true;
    bool ok = m_packet->finalize();
    m_packetOpen = false;
    return ok;
}

bool DataStreamWriter::hasError() const
{
    return m_packet && m_packet->hasError();
}

...
static Utils::Result<void> checkNoExtensions(const QJsonObject &obj, int idx)
{
    ...
    if (e.isObject() && !e.toObject().isEmpty())
        return Utils::ResultError(parseErr(
            u"unsupported extension(s) present (this consumer supports none)"_s, -1, idx));
...
static Utils::Result<void> validateRootRoles(const FieldClassPtr &rootFc, const Schema &schema,
                                             RootScope scope, bool dscHasClock, int idx);
...
Utils::Result<Schema> MetadataReader::read()
{
    ...
    if (errorOffset >= 0)
        return Utils::ResultError(parseErr(u"Expected RS byte (0x1E) at start of fragment"_s, errorOffset));
    ...
    for (int idx = 0; idx < fragments.size(); ++idx) {
        ...
        if (doc.isNull())
            return Utils::ResultError(parseErr(u"JSON parse error: "_s + jsonErr.errorString(), -1, idx));
        ...
        if (type == u"preamble") {
            if (++preambleCount > 1)
                return Utils::ResultError(parseErr(u"Metadata stream must contain exactly one preamble fragment"_s, -1, idx));
            auto r = parsePreamble(obj, schema, idx);
            if (!r) return Utils::ResultError(r.error());
        } else if (type == u"field-class-alias") {
            auto r = parseFieldClassAlias(obj, schema, idx);
            if (!r) return Utils::ResultError(r.error());
        } else if (type == u"trace-class") {
            if (hasTraceClass)
                return Utils::ResultError(parseErr(u"metadata stream must contain at most one trace-class fragment"_s, -1, idx));
            if (hasDataStreamClass)
                return Utils::ResultError(parseErr(u"trace-class fragment must precede any data-stream-class fragment"_s, -1, idx));
...
Utils::Result<void> MetadataReader::parsePreamble(const QJsonObject &obj,
                                                   Schema &schema, int idx)
{
    ...
    if (obj.contains(u"uuid")) {
        ...
        if (!uuidVal.isArray())
            return Utils::ResultError(parseErr(
                u"Preamble uuid must be an array of 16 integers"_s, -1, idx));
        const QJsonArray arr = uuidVal.toArray();
        if (arr.size() != 16)
            return Utils::ResultError(parseErr(
                u"Preamble uuid must have exactly 16 elements"_s, -1, idx));
        QByteArray bytes(16, '\0');
        for (int i = 0; i < 16; ++i)
            bytes[i] = static_cast<char>(arr[i].toInt());
...
Utils::Result<void> MetadataReader::parseFieldClassAlias(const QJsonObject &obj,
                                                          Schema &schema, int idx)
{
    if (auto r = checkNoExtensions(obj, idx); !r) return Utils::ResultError(r.error());
    ...
    if (schema.fieldClassAliases.contains(name))
        return Utils::ResultError(parseErr(u"duplicate field-class-alias name: %1"_s.arg(name), -1, idx));
...
Utils::Result<void>
MetadataReader::parseRootStructureFieldClass(const QJsonObject &obj, const QString &key,
                                             const Schema &schema, int idx,
                                             FieldClassPtr &out) const
{
    ...
    if (!*r || (*r)->kind != FieldClassKind::Structure)
        return Utils::ResultError(parseErr(u"%1 must be a structure field class"_s.arg(key), -1, idx));
    out = std::move(*r);
    return Utils::ResultOk;
}

Utils::Result<void> MetadataReader::parseTraceClass(const QJsonObject &obj,
                                                     Schema &schema, int idx)
{
    if (auto r = checkNoExtensions(obj, idx); !r) return Utils::ResultError(r.error());
    TraceClass tc;
    ...
    if (auto r = parseRootStructureFieldClass(obj, u"packet-header-field-class"_s, schema, idx,
                                              tc.packetHeaderFieldClass); !r)
        return Utils::ResultError(r.error());
    if (auto r = validateRootRoles(tc.packetHeaderFieldClass, schema, RootScope::TracePacketHeader,
                                   /*dscHasClock=*/false, idx); !r)
        return Utils::ResultError(r.error());
    schema.traceClass = std::move(tc);
    return Utils::ResultOk;
}

Utils::Result<void> MetadataReader::parseClockClass(const QJsonObject &obj,
                                                     Schema &schema, int idx)
{
    if (auto r = checkNoExtensions(obj, idx); !r) return Utils::ResultError(r.error());
    ClockClass cc;
    ...
    for (const auto &existing : schema.clockClasses) {
        if (existing.id == cc.id)
            return Utils::ResultError(parseErr(u"duplicate clock-class id: %1"_s.arg(cc.id), -1, idx));
    }
    ...
    if (freq <= 0)
        return Utils::ResultError(parseErr(u"clock-class frequency must be greater than zero"_s, -1, idx));
    ...
    if (obj.contains(u"offset-from-origin")) {
        ...
        if (cycles < 0)
            return Utils::ResultError(parseErr(u"clock-class offset cycles must be >= 0"_s, -1, idx));
        if (static_cast<quint64>(cycles) >= cc.frequency)
            return Utils::ResultError(parseErr(u"clock-class offset cycles must be less than frequency"_s, -1, idx));
    ...
    if (obj.contains(u"origin")) {
        ...
        if (origin.isString()) {
            if (origin.toString() != u"unix-epoch")
                return Utils::ResultError(parseErr(
                    u"clock-class origin string must be \"unix-epoch\""_s, -1, idx));
            ...
            if (cc.origin.name.isEmpty() || cc.origin.uid.isEmpty())
                return Utils::ResultError(parseErr(
                    u"clock-class custom origin must have name and uid"_s, -1, idx));
        } else {
            return Utils::ResultError(parseErr(
...
Utils::Result<void> MetadataReader::parseDataStreamClass(const QJsonObject &obj,
                                                          Schema &schema, int idx)
{
    if (auto r = checkNoExtensions(obj, idx); !r) return Utils::ResultError(r.error());
    DataStreamClass dsc;
    dsc.attributes = parseAttributes(obj);
    const double idd = obj[u"id"].toDouble(0);
    if (idd < 0)
    ...
    for (const auto &existing : schema.dataStreamClasses) {
        if (existing.id == dsc.id)
            return Utils::ResultError(parseErr(u"duplicate data-stream-class id: %1"_s.arg(dsc.id), -1, idx));
    }
    ...
    if (!dsc.defaultClockClassName.isEmpty()) {
        ...
        const bool found = std::any_of(schema.clockClasses.cbegin(), schema.clockClasses.cend(),
                                       [&](const ClockClass &cc) { return cc.id == ref; });
        if (!found)
            return Utils::ResultError(parseErr(
    ...
    if (auto r = parseRootStructureFieldClass(obj, u"packet-context-field-class"_s, schema, idx,
                                              dsc.packetContextFieldClass); !r)
        return Utils::ResultError(r.error());
    if (auto r = parseRootStructureFieldClass(obj, u"event-record-header-field-class"_s, schema, idx,
                                              dsc.eventRecordHeaderFieldClass); !r)
        return Utils::ResultError(r.error());
    if (auto r = parseRootStructureFieldClass(obj, u"event-record-common-context-field-class"_s, schema, idx,
                                              dsc.eventRecordCommonContextFieldClass); !r)
        return Utils::ResultError(r.error());

    if (auto r = validateRootRoles(dsc.packetContextFieldClass, schema,
                                   RootScope::DataStreamPacketContext, dscHasClock, idx); !r)
        return Utils::ResultError(r.error());
    if (auto r = validateRootRoles(dsc.eventRecordHeaderFieldClass, schema,
                                   RootScope::EventRecordHeader, dscHasClock, idx); !r)
        return Utils::ResultError(r.error());
...
Utils::Result<void> MetadataReader::parseEventRecordClass(const QJsonObject &obj,
                                                           Schema &schema, int idx)
{
    if (auto r = checkNoExtensions(obj, idx); !r) return Utils::ResultError(r.error());
    EventRecordClass erc;
    erc.attributes = parseAttributes(obj);
    const double ercIdd = obj[u"id"].toDouble(0);
    if (ercIdd < 0)
    ...
    if (auto r = parseRootStructureFieldClass(obj, u"specific-context-field-class"_s, schema, idx,
                                              erc.specificContextFieldClass); !r)
        return Utils::ResultError(r.error());
    if (auto r = parseRootStructureFieldClass(obj, u"payload-field-class"_s, schema, idx,
                                              erc.payloadFieldClass); !r)
        return Utils::ResultError(r.error());

    const double dscIdd = obj[u"data-stream-class-id"].toDouble(0);
    if (dscIdd < 0)
        return Utils::ResultError(parseErr(u"event-record-class data-stream-class-id must be >= 0"_s, -1, idx));
...
static Utils::Result<ByteOrder> parseByteOrder(const QJsonObject &obj, int idx)
{
    ...
    if (!v.isString())
        return Utils::ResultError(parseErr(u"byte-order is required and must be a string"_s, -1, idx));
    const QString s = v.toString();
    if (s == u"big-endian")    return ByteOrder::BigEndian;
    if (s == u"little-endian") return ByteOrder::LittleEndian;
    return Utils::ResultError(parseErr(
        u"byte-order must be \"big-endian\" or \"little-endian\""_s, -1, idx));
}
...
static Utils::Result<DisplayBase> parseDisplayBase(const QJsonValue &v, int idx)
{
    ...
    switch (v.toInt()) {
    case 2:  return DisplayBase::Binary;
    case 8:  return DisplayBase::Octal;
    case 10: return DisplayBase::Decimal;
    case 16: return DisplayBase::Hexadecimal;
    }
    return Utils::ResultError(parseErr(
        u"preferred-display-base must be one of 2, 8, 10, 16"_s, -1, idx));
}
...
static Utils::Result<StringEncoding> parseStringEncoding(const QJsonValue &v, int idx)
{
    ...
    const QString s = v.toString();
    if (s == u"utf-8")    return StringEncoding::Utf8;
    if (s == u"utf-16be") return StringEncoding::Utf16Be;
    if (s == u"utf-16le") return StringEncoding::Utf16Le;
    if (s == u"utf-32be") return StringEncoding::Utf32Be;
    if (s == u"utf-32le") return StringEncoding::Utf32Le;
    return Utils::ResultError(parseErr(
        u"encoding must be one of utf-8, utf-16be, utf-16le, utf-32be, utf-32le"_s, -1, idx));
}

static UIntRole parseUIntRole(const QString &s)
{
    if (s == u"packet-magic-number")                     return UIntRole::PacketMagicNumber;
    if (s == u"event-record-class-id")                   return UIntRole::EventRecordClassId;
    if (s == u"data-stream-id")                          return UIntRole::DataStreamId;
    if (s == u"data-stream-class-id")                    return UIntRole::DataStreamClassId;
    if (s == u"packet-total-length")                     return UIntRole::PacketTotalLength;
    if (s == u"packet-content-length")                   return UIntRole::PacketContentLength;
    if (s == u"default-clock-timestamp")                 return UIntRole::DefaultClockTimestamp;
    if (s == u"packet-end-default-clock-timestamp")      return UIntRole::PacketEndDefaultClockTimestamp;
    if (s == u"discarded-event-record-counter-snapshot") return UIntRole::DiscardedEventRecordCounterSnapshot;
    if (s == u"packet-sequence-number")                  return UIntRole::PacketSequenceNumber;
    return UIntRole::None;
}
...
static bool roleAllowedInScope(UIntRole role, RootScope scope)
{
    ...
    case RootScope::TracePacketHeader:
        return role == UIntRole::DataStreamClassId
            || role == UIntRole::DataStreamId
    ...
    case RootScope::DataStreamPacketContext:
        return role == UIntRole::DefaultClockTimestamp
            ...
            || role == UIntRole::PacketSequenceNumber
            || role == UIntRole::PacketTotalLength;
    case RootScope::EventRecordHeader:
        return role == UIntRole::DefaultClockTimestamp
            || role == UIntRole::EventRecordClassId;
    }
    return false;
}
...
{
    switch (role) {
    case UIntRole::PacketMagicNumber:                     return u"packet-magic-number"_s;
    case UIntRole::EventRecordClassId:                    return u"event-record-class-id"_s;
    case UIntRole::DataStreamId:                          return u"data-stream-id"_s;
    case UIntRole::DataStreamClassId:                     return u"data-stream-class-id"_s;
    case UIntRole::PacketTotalLength:                     return u"packet-total-length"_s;
    case UIntRole::PacketContentLength:                   return u"packet-content-length"_s;
    case UIntRole::DefaultClockTimestamp:                 return u"default-clock-timestamp"_s;
    case UIntRole::PacketEndDefaultClockTimestamp:        return u"packet-end-default-clock-timestamp"_s;
    case UIntRole::DiscardedEventRecordCounterSnapshot:   return u"discarded-event-record-counter-snapshot"_s;
    case UIntRole::PacketSequenceNumber:                  return u"packet-sequence-number"_s;
    case UIntRole::None:                                  break;
    }
    return u"(unknown)"_s;
}
                                             ...
                                             RootScope scope, bool dscHasClock, int idx)
{
    ...
    for (int i = 0; i < sfc.members.size(); ++i) {
        ...
        if (mfc->kind == FieldClassKind::FixedLengthUInt) {
            ...
            roles = u.roles; fixedLen = u.length; isFixedUint = true;
        ...
        for (UIntRole role : roles) {
            ...
            if (!roleAllowedInScope(role, scope))
                return Utils::ResultError(parseErr(
                    u"role %1 is not allowed in this root structure"_s.arg(roleName(role)), -1, idx));
            hasValidRole = true;
            switch (role) {
            case UIntRole::PacketMagicNumber:
                if (!isFixedUint || fixedLen != 32)
                    return Utils::ResultError(parseErr(
                        u"packet-magic-number role requires a 32-bit fixed-length unsigned integer"_s, -1, idx));
                if (i != 0)
                    return Utils::ResultError(parseErr(
                        u"packet-magic-number field must be the first packet header member"_s, -1, idx));
                break;
            case UIntRole::DefaultClockTimestamp:
            case UIntRole::PacketEndDefaultClockTimestamp:
                if (!dscHasClock)
                    return Utils::ResultError(parseErr(
                        u"clock-timestamp role requires the data stream class to have a default-clock-class-id"_s, -1, idx));
                break;
            default:
                break;
            }
        ...
        if (mfc->kind == FieldClassKind::StaticLengthBlob) {
            ...
            if (b.roles.contains(BlobRole::MetadataStreamUuid)) {
                ...
                if (scope != RootScope::TracePacketHeader)
                    return Utils::ResultError(parseErr(
                        u"metadata-stream-uuid role is only allowed in the packet header"_s, -1, idx));
                if (b.length != 16)
                    return Utils::ResultError(parseErr(
                        u"metadata-stream-uuid role requires a static-length-blob of length 16"_s, -1, idx));
                if (!schema.uuid || schema.uuid->isNull())
                    return Utils::ResultError(parseErr(
                        u"metadata-stream-uuid role requires the preamble to declare a uuid"_s, -1, idx));
                hasBlobRole = true;
            }
        }
            ...
            if (!ok)
                return Utils::ResultError(parseErr(
                    u"member '%1' has no valid role and is not a structure, optional, variant, "
                     "or potential selector/length field"_s.arg(sfc.members[i].name), -1, idx));
        }
    }
    return Utils::ResultOk;
}
...
Utils::Result<FieldLocation>
MetadataReader::parseFieldLocation(const QJsonObject &obj, int idx) const
{
    FieldLocation loc;
    ...
    if (originVal.isString()) {
        ...
        if (originStr == u"packet-header")
            ...
            return Utils::ResultError(parseErr(
                u"Invalid field-location origin: %1"_s.arg(originStr), -1, idx));
    }

    if (!obj[u"path"].isArray() || obj[u"path"].toArray().isEmpty())
        return Utils::ResultError(parseErr(u"field-location path must be a non-empty array"_s, -1, idx));
    ...
    for (const auto &seg : path) {
        if (seg.isNull())
            ...
            return Utils::ResultError(parseErr(
                u"field-location path elements must be strings or null"_s, -1, idx));
    }
    if (!loc.path.last().has_value())
        return Utils::ResultError(parseErr(u"last field-location path element must not be null"_s, -1, idx));
    return loc;
}

Utils::Result<FieldClassPtr>
MetadataReader::parseFieldClass(const QJsonValue &val, const Schema &schema, int idx) const
{
    ...
    if (val.isString()) {
        ...
        if (it == schema.fieldClassAliases.end())
            return Utils::ResultError(parseErr(u"Unknown field class alias: %1"_s.arg(name), -1, idx));
        return *it;
    }
    ...
    auto checkLengthAlignment = [&](int length, int alignment) -> Utils::Result<void> {
        ...
        if (length > 64)
            return Utils::ResultError(parseErr(
                u"fixed-length bit array lengths greater than 64 bits are not supported"_s, -1, idx));
        if (!isPositivePowerOfTwo(alignment))
            return Utils::ResultError(parseErr(u"alignment must be a positive power of two"_s, -1, idx));
        return Utils::ResultOk;
    };
    ...
    if (type == u"fixed-length-bit-array") {
        if (auto r = requireLength(); !r) return Utils::ResultError(r.error());
        auto f = std::make_shared<FixedLengthBitArrayFC>();
        f->attributes = parseAttributes(obj);
        f->length    = obj[u"length"].toInt(0);
        auto bo = parseByteOrder(obj, idx); if (!bo) return Utils::ResultError(bo.error());
        f->byteOrder = *bo;
        f->bitOrder  = resolveBitOrder(obj, *bo);
        f->alignment = obj[u"alignment"].toInt(1);
        if (auto r = checkLengthAlignment(f->length, f->alignment); !r) return Utils::ResultError(r.error());
        return f;
    }
    if (type == u"fixed-length-bit-map") {
        ...
        if (flags.isEmpty())
            return Utils::ResultError(parseErr(u"fixed-length-bit-map flags must contain one or more properties"_s, -1, idx));
        for (auto it = flags.begin(); it != flags.end(); ++it) {
            QList<QPair<qint64, qint64>> ranges;
            if (!parseIntegerRangeSet(it.value(), ranges))
                return Utils::ResultError(parseErr(u"invalid bit index range set for flag %1"_s.arg(it.key()), -1, idx));
    ...
    if (type == u"fixed-length-unsigned-integer") {
        ...
        f->bitOrder    = resolveBitOrder(obj, *bo);
        f->alignment   = obj[u"alignment"].toInt(1);
        auto db = parseDisplayBase(obj[u"preferred-display-base"], idx); if (!db) return Utils::ResultError(db.error());
        f->displayBase = *db;
        f->mappings    = parseMappings(obj);
        if (auto r = checkLengthAlignment(f->length, f->alignment); !r) return Utils::ResultError(r.error());
        for (const auto &rv : obj[u"roles"].toArray())
            f->roles.append(parseUIntRole(rv.toString()));
        return f;
    }
    if (type == u"fixed-length-signed-integer") {
        ...
        if (obj.contains(u"roles"))
            return Utils::ResultError(parseErr(u"roles may only exist on an unsigned integer field class"_s, -1, idx));
        return f;
    }
    if (type == u"fixed-length-floating-point-number") {
        ...
        const bool validLen = f->length == 16 || f->length == 32 || f->length == 64
                              || f->length == 128 || (f->length > 128 && f->length % 32 == 0);
        if (!validLen)
            return Utils::ResultError(parseErr(
                u"floating point length must be 16, 32, 64, 128, or a multiple of 32 greater than 128"_s, -1, idx));
        return f;
    }
    ...
    if (type == u"null-terminated-string") {
        ...
        f->attributes = parseAttributes(obj);
        auto enc = parseStringEncoding(obj[u"encoding"], idx); if (!enc) return Utils::ResultError(enc.error());
        f->encoding = *enc;
        return f;
    }
    if (type == u"static-length-string") {
        ...
        if (f->length < 0)
            return Utils::ResultError(parseErr(u"static-length-string length must be present and >= 0"_s, -1, idx));
        auto enc = parseStringEncoding(obj[u"encoding"], idx); if (!enc) return Utils::ResultError(enc.error());
        f->encoding = *enc;
        return f;
    }
    if (type == u"dynamic-length-string") {
        ...
        if (!locResult) return Utils::ResultError(locResult.error());
    ...
    if (type == u"static-length-blob") {
        ...
        f->length    = obj[u"length"].toInteger(-1);
        if (f->length < 0)
            return Utils::ResultError(parseErr(u"static-length-blob length must be present and >= 0"_s, -1, idx));
    ...
    if (type == u"structure") {
        ...
        if (!isPositivePowerOfTwo(f->minimumAlignment))
            return Utils::ResultError(parseErr(u"minimum-alignment must be a positive power of two"_s, -1, idx));
        ...
        for (const auto &mVal : members) {
            ...
            if (auto r = checkNoExtensions(mObj, idx); !r) return Utils::ResultError(r.error());
            const QString memberName = mObj[u"name"].toString();
            if (memberName.isEmpty())
                return Utils::ResultError(parseErr(u"structure member class missing name"_s, -1, idx));
            if (memberNames.contains(memberName))
                return Utils::ResultError(parseErr(u"duplicate structure member name: %1"_s.arg(memberName), -1, idx));
            memberNames.insert(memberName);
            if (!mObj.contains(u"field-class"))
                return Utils::ResultError(parseErr(u"structure member %1 missing field-class"_s.arg(memberName), -1, idx));
            auto memberFc = parseFieldClass(mObj[u"field-class"], schema, idx);
            if (!memberFc) return Utils::ResultError(memberFc.error());
    ...
    if (type == u"static-length-array") {
        ...
        f->attributes = parseAttributes(obj);
        if (!obj.contains(u"length"))
            return Utils::ResultError(parseErr(u"static-length-array missing required length"_s, -1, idx));
        f->length = obj[u"length"].toInteger(-1);
        if (f->length < 0)
            return Utils::ResultError(parseErr(u"static-length-array length must be present and >= 0"_s, -1, idx));
        f->minimumAlignment = obj[u"minimum-alignment"].toInt(1);
        if (!isPositivePowerOfTwo(f->minimumAlignment))
            return Utils::ResultError(parseErr(u"minimum-alignment must be a positive power of two"_s, -1, idx));
        if (!obj.contains(u"element-field-class"))
            return Utils::ResultError(parseErr(u"static-length-array missing element-field-class"_s, -1, idx));
        auto elem = parseFieldClass(obj[u"element-field-class"], schema, idx);
        if (!elem) return Utils::ResultError(elem.error());
    ...
    if (type == u"dynamic-length-array") {
        ...
        f->lengthFieldLocation = std::move(*locResult);
        if (!obj.contains(u"element-field-class"))
            return Utils::ResultError(parseErr(u"dynamic-length-array missing element-field-class"_s, -1, idx));
        auto elem = parseFieldClass(obj[u"element-field-class"], schema, idx);
        if (!elem) return Utils::ResultError(elem.error());
        f->elementFieldClass = std::move(*elem);
        return f;
    ...
    if (type == u"variant") {
        ...
        if (options.isEmpty())
            return Utils::ResultError(parseErr(u"variant options must contain one or more elements"_s, -1, idx));
        ...
        struct TaggedRange { int option; qint64 lo, hi; };
        QList<TaggedRange> allSelectorRanges;
        int optionIndex = 0;
        for (const auto &optVal : options) {
            const QJsonObject optObj = optVal.toObject();
            if (auto r = checkNoExtensions(optObj, idx); !r) return Utils::ResultError(r.error());
            VariantOption opt;
            opt.attributes = parseAttributes(optObj);
            opt.name = optObj[u"name"].toString();
            if (!opt.name.isEmpty()) {
                if (optionNames.contains(opt.name))
                    return Utils::ResultError(parseErr(u"duplicate variant option name: %1"_s.arg(opt.name), -1, idx));
                optionNames.insert(opt.name);
            }
            if (!optObj.contains(u"field-class"))
                return Utils::ResultError(parseErr(u"variant option missing field-class"_s, -1, idx));
            auto optFc = parseFieldClass(optObj[u"field-class"], schema, idx);
            if (!optFc) return Utils::ResultError(optFc.error());
            opt.fieldClass = std::move(*optFc);
            QList<QPair<qint64, qint64>> ranges;
            if (!parseIntegerRangeSet(optObj[u"selector-field-ranges"], ranges))
                return Utils::ResultError(parseErr(u"variant option missing or invalid selector-field-ranges"_s, -1, idx));
            for (const auto &p : ranges) {
    ...
    if (type == u"optional") {
        ...
        auto inner = parseFieldClass(obj[u"field-class"], schema, idx);
        if (!inner) return Utils::ResultError(inner.error());
        ...
        if (obj.contains(u"selector-field-ranges")) {
            ...
            if (!parseIntegerRangeSet(obj[u"selector-field-ranges"], ranges))
                return Utils::ResultError(parseErr(u"invalid optional selector-field-ranges"_s, -1, idx));
            for (const auto &p : ranges) {
                if (p.first == p.second)

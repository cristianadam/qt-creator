...
bool MetadataWriter::writeFragment(const QByteArray &json)
{
    if (m_dev->write(&RS, 1) != 1)  { m_error = true; return false; }
    if (m_dev->write(json) != json.size()) { m_error = true; return false; }
    if (m_dev->write(&LF, 1) != 1)  { m_error = true; return false; }
    return true;
}

bool MetadataWriter::write(const Schema &schema)
{
    if (!writePreamble(schema))
        return false;
    ...
    for (const QString &name : aliasNames) {
        ...
        if (it == schema.fieldClassAliases.constEnd())
            continue;
        if (!writeFieldClassAlias(name, *it.value()))
            return false;
    }

    if (schema.traceClass)
        if (!writeTraceClass(*schema.traceClass))
            return false;

    for (const auto &cc : schema.clockClasses)
        if (!writeClockClass(cc))
            return false;

    for (const auto &dsc : schema.dataStreamClasses) {
        if (!writeDataStreamClass(dsc))
            return false;
        for (const auto &erc : dsc.eventRecordClasses)
            if (!writeEventRecordClass(erc, dsc.id))
                return false;
    }
...
bool MetadataWriter::writePreamble(const Schema &schema)
{
    QJsonObject obj;
...
bool MetadataWriter::writeFieldClassAlias(const QString &name, const FieldClass &fc)
{
    QJsonObject obj;
...
bool MetadataWriter::writeTraceClass(const TraceClass &tc)
{
    QJsonObject obj;
...
bool MetadataWriter::writeClockClass(const ClockClass &cc)
{
    QJsonObject obj;
...
bool MetadataWriter::writeDataStreamClass(const DataStreamClass &dsc)
{
    QJsonObject obj;
    ...
    if (dsc.eventRecordCommonContextFieldClass)
        obj[u"event-record-common-context-field-class"] = fieldClassToJson(*dsc.eventRecordCommonContextFieldClass);
    writeAttributes(obj, dsc.attributes);
    return writeFragment(QJsonDocument(obj).toJson(QJsonDocument::Compact));
}

bool MetadataWriter::writeEventRecordClass(const EventRecordClass &erc, quint64 dataStreamClassId)
{
    QJsonObject obj;
...
{
    switch (db) {
    case DisplayBase::Binary:      return 2;
    case DisplayBase::Octal:       return 8;
    case DisplayBase::Hexadecimal: return 16;
    default:                       return 10;
    }
}
...
{
    switch (enc) {
    case StringEncoding::Utf16Be: return u"utf-16be"_s;
    case StringEncoding::Utf16Le: return u"utf-16le"_s;
    case StringEncoding::Utf32Be: return u"utf-32be"_s;
    case StringEncoding::Utf32Le: return u"utf-32le"_s;
    default:                      return u"utf-8"_s;
    }
}

// Serialize discrete selector values and ranges as an integer-range-set.
static QJsonArray selectorRangeSet(const QList<qint64> &values,
                                   const QList<VariantSelectorRange> &ranges)
{
    QJsonArray out;
    for (auto v : values) {
        QJsonArray r;
...
{
    switch (o) {
    case FieldLocation::Origin::PacketHeader:               return u"packet-header"_s;
    case FieldLocation::Origin::PacketContext:              return u"packet-context"_s;
    case FieldLocation::Origin::EventRecordHeader:          return u"event-record-header"_s;
    case FieldLocation::Origin::EventRecordCommonContext:   return u"event-record-common-context"_s;
    case FieldLocation::Origin::EventRecordSpecificContext: return u"event-record-specific-context"_s;
    default:                                                return u"event-record-payload"_s;
    }
}
...
QJsonObject MetadataWriter::fieldClassToJson(const FieldClass &fc) const
{
    QJsonObject obj;
    ...
    case FieldClassKind::Structure: {
        ...
        for (const auto &m : f.members) {
            QJsonObject mObj;
            mObj[u"name"]        = m.name;
            mObj[u"field-class"] = fieldClassToJson(*m.fieldClass);
    ...
    case FieldClassKind::StaticLengthArray: {
        ...
        obj[u"length"]              = f.length;
        obj[u"minimum-alignment"]   = f.minimumAlignment;
        obj[u"element-field-class"] = fieldClassToJson(*f.elementFieldClass);
        break;
    }
    ...
    case FieldClassKind::Variant: {
        ...
        for (const auto &opt : f.options) {
            ...
            optObj[u"field-class"] = fieldClassToJson(*opt.fieldClass);
            optObj[u"selector-field-ranges"] = selectorRangeSet(opt.selectorValues, opt.selectorRanges);
    ...
    case FieldClassKind::Optional: {
        ...
        obj[u"field-class"]             = fieldClassToJson(*f.fieldClass);
        obj[u"selector-field-location"] = fieldLocationToJson(f.selectorFieldLocation);

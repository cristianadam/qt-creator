...
PacketWriter::PacketWriter(QIODevice *dev,
                           const DataStreamClass &dsc,
                           const TraceClass *tc,
                           std::optional<QUuid> uuid)
    : m_dev(dev), m_dsc(dsc), m_tc(tc), m_uuid(uuid)
{}
...
static StructureValue makeEventHeader(const StructureFC &hfc,
                                      ...
                                      quint64 timestamp)
{
    ...
    if (var) {
        ...
        if (chosen < 0) {
            for (int i = 0; i < var->options.size(); ++i) {
                const VariantOption &opt = var->options[i];
                if (!memberNameWithRole(opt.fieldClass.get(), UIntRole::EventRecordClassId).isEmpty()) {
                    chosen = i;
...
static RoleFieldPos findRoleField(const FieldClass *fc,
                                  UIntRole role,
                                  qint64 baseOffset = 0)
{
    if (!fc || fc->kind != FieldClassKind::Structure)
...
void PacketWriter::beginPacket(const StructureValue &packetContext)
{
    ...
    delete m_buf;
    auto *qbuf = new QBuffer(&m_scratch);
    qbuf->open(QIODevice::WriteOnly);
    m_buf = new BitBuffer(qbuf);
    ...
    if (m_tc && m_tc->packetHeaderFieldClass) {
        ...
        FieldWriter(*m_buf, makeResolver(packetContext))
            .write(*phfc, makeStructureValue(hdr));
    }
                                    ...
                                    quint64 timestamp)
{
    ...
    if (m_dsc.eventRecordCommonContextFieldClass)
        FieldWriter(*m_buf, resolver).write(*m_dsc.eventRecordCommonContextFieldClass,
                                           makeStructureValue(commonCtx));

    const EventRecordClass *erc = nullptr;
    for (const auto &e : m_dsc.eventRecordClasses) {
        if (e.id == eventClassId) { erc = &e; break; }
    }

    if (erc) {
        if (erc->specificContextFieldClass)
            FieldWriter(*m_buf, resolver).write(*erc->specificContextFieldClass,
                                               makeStructureValue(specificCtx));
        if (erc->payloadFieldClass)
            FieldWriter(*m_buf, resolver).write(*erc->payloadFieldClass,
                                               makeStructureValue(payload));
    }
}

bool PacketWriter::finalize()
{
    if (!m_inPacket || m_error)
        return false;
    ...
    if (m_totalLength.found())
        m_buf->patchBits(m_totalLength.bitOffset, static_cast<quint64>(totalBits),
                         m_totalLength.length, m_totalLength.byteOrder, m_totalLength.bitOrder);
    if (m_contentLength.found())
        m_buf->patchBits(m_contentLength.bitOffset, static_cast<quint64>(contentBits),
                         m_contentLength.length, m_contentLength.byteOrder, m_contentLength.bitOrder);
    ...
    if (m_beginClock.found() && m_firstTimestamp)
        m_buf->patchBits(m_beginClock.bitOffset, *m_firstTimestamp,
                         m_beginClock.length, m_beginClock.byteOrder, m_beginClock.bitOrder);
    if (m_endClock.found() && m_lastTimestamp)
        m_buf->patchBits(m_endClock.bitOffset, *m_lastTimestamp,
                         m_endClock.length, m_endClock.byteOrder, m_endClock.bitOrder);
...
FieldLocationResolver PacketWriter::makeResolver(const StructureValue &packetCtx) const
{
    return [&packetCtx](const FieldLocation &loc) -> std::optional<quint64> {
        if (loc.path.isEmpty()) return std::nullopt;
        ...
        if (std::holds_alternative<quint64>(*fv)) return std::get<quint64>(*fv);
        if (std::holds_alternative<qint64>(*fv)) return static_cast<quint64>(std::get<qint64>(*fv));
        return std::nullopt;
    };
}

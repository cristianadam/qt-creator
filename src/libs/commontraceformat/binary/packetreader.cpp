...
PacketReader::PacketReader(QIODevice *dev,
...
RoleField findUIntRole(const FieldClass *fc, const StructureValue &sv, UIntRole role,
...
quint64 extractRole(const StructureValue &sv, const FieldClass *fc, UIntRole role,
                    const QHash<QString, int> &clockBits = {})
{
...
selectedVariantOption(const FieldClass *headerFc, const StructureValue &header)
{
    ...
    for (const auto &member : sfc.members) {
        ...
        if (!fv || !std::holds_alternative<std::shared_ptr<VariantValue>>(*fv))
            continue;
        const auto &vv = *std::get<std::shared_ptr<VariantValue>>(*fv);
        ...
        for (const auto &opt : vfc.options) {
            if (opt.name == vv.selectedOption && opt.fieldClass
                && opt.fieldClass->kind == FieldClassKind::Structure)
                return std::pair{opt.fieldClass.get(),
                                 *std::get<std::shared_ptr<StructureValue>>(*vv.value)};
        }
    }
    return std::nullopt;
}
...
Utils::Result<> PacketReader::openPacket()
{
    ...
    QByteArray initial = m_dev->read(512);
    if (initial.isEmpty())
    ...
    if (m_dsc->packetContextFieldClass) {
        ...
        quint64 totalBits = extractRole(ctxValue, m_dsc->packetContextFieldClass.get(),
                                        UIntRole::PacketTotalLength, fr.rootMemberClockBits());
        if (totalBits > 0)
    ...
    if (totalBytes > static_cast<quint64>(initial.size())) {
        ...
        // Over-read: trim to packet boundary and seek device back
    ...
    if (m_tc && m_tc->packetHeaderFieldClass) {
        ...
        auto hdr = fr.read(*m_tc->packetHeaderFieldClass);
        if (!hdr) return Utils::ResultError(hdr.error());
        if (isStructure(*hdr))
        ...
        const RoleField magic = findUIntRole(m_tc->packetHeaderFieldClass.get(),
                                             m_pktHeader, UIntRole::PacketMagicNumber,
        ...
        const RoleField dsId = findUIntRole(m_tc->packetHeaderFieldClass.get(),
                                            m_pktHeader, UIntRole::DataStreamId,
                                            fr.rootMemberClockBits());
        if (dsId.found)
            m_dsId = dsId.value;
        ...
        if (m_schema && m_schema->uuid && !m_schema->uuid->isNull()) {
            const auto uuidBytes = findBlobRole(m_tc->packetHeaderFieldClass.get(),
                                                m_pktHeader, BlobRole::MetadataStreamUuid);
    ...
    if (m_dsc->packetContextFieldClass) {
        ...
        auto ctxResult = fr.read(*m_dsc->packetContextFieldClass);
        if (!ctxResult) return Utils::ResultError(ctxResult.error());
        ...
        const RoleField totalRole   = findUIntRole(cfc, m_pktContext, UIntRole::PacketTotalLength, clockBits);
        const RoleField contentRole = findUIntRole(cfc, m_pktContext, UIntRole::PacketContentLength, clockBits);
        if (totalRole.found)   totalBits   = totalRole.value;
        if (contentRole.found) contentBits = contentRole.value;

        // Initialize DEF_CLK_VAL from the packet beginning timestamp, if any.
        const RoleField begin = findUIntRole(cfc, m_pktContext,
                                             UIntRole::DefaultClockTimestamp, clockBits);
        if (begin.found)
        ...
        const RoleField end = findUIntRole(cfc, m_pktContext,
                                           UIntRole::PacketEndDefaultClockTimestamp, clockBits);
        if (end.found) {
        ...
        const RoleField disc = findUIntRole(cfc, m_pktContext,
                                            UIntRole::DiscardedEventRecordCounterSnapshot, clockBits);
        if (disc.found) m_discErSnap = disc.value;
        const RoleField seq = findUIntRole(cfc, m_pktContext,
                                           UIntRole::PacketSequenceNumber, clockBits);
        if (seq.found) m_pktSeqNum = seq.value;
    }

    // Reconcile total/content lengths (spec 6.1): each defaults to the other
    // when only one is given; content must not exceed total.
    if (!totalBits && contentBits)   totalBits = contentBits;
    if (!contentBits && totalBits)   contentBits = totalBits;
    if (totalBits && contentBits && *contentBits > *totalBits)
        return Utils::ResultError(u"Packet content length exceeds total length"_s);

    m_contentEndBit = contentBits ? static_cast<qint64>(*contentBits)
                                  : m_packetData.size() * 8LL;
    if (m_contentEndBit > m_packetData.size() * 8LL)
...
Utils::Result<EventRecord> PacketReader::nextEvent()
{
    ...
    if (m_dsc->eventRecordHeaderFieldClass) {
        ...
        const RoleField idRole = findUIntRole(hfc, rec.header, UIntRole::EventRecordClassId, clockBits);
        if (idRole.found)
        ...
        const RoleField tsRole = findUIntRole(hfc, rec.header, UIntRole::DefaultClockTimestamp, clockBits);
        if (tsRole.found)
        ...
        if (auto opt = selectedVariantOption(hfc, rec.header)) {
            ...
            if (extTs.found) {
                ...
                updateClockValue(m_defClkVal, extTs.value,
                                 extTs.lengthBits > 0 ? extTs.lengthBits : 64);
                rec.timestamp = m_defClkVal;
            }
        }
            ...
            if (m_prevEventTimestamp && rec.timestamp < *m_prevEventTimestamp)
                return Utils::ResultError(
                    u"Event record timestamp %1 is less than the previous timestamp %2"_s
                        .arg(rec.timestamp).arg(*m_prevEventTimestamp));
    ...
    if (!rec.eventClass)
        return Utils::ResultError(
            u"No event record class with id %1 in data stream class %2"_s
                .arg(rec.eventClassId).arg(m_dsc->id));

    {
        if (rec.eventClass->specificContextFieldClass) {
            auto r = FieldReader(*m_buf, makeResolver(), FieldLocation::Origin::EventRecordSpecificContext)
                         .read(*rec.eventClass->specificContextFieldClass);
            if (!r) return Utils::ResultError(r.error());
            if (isStructure(*r)) rec.specificContext = asStructure(*r);
        ...
        if (rec.eventClass->payloadFieldClass) {
            ...
            if (!r) return Utils::ResultError(r.error());
            if (isStructure(*r)) rec.payload = asStructure(*r);
        }
    }
...
Utils::Result<> PacketReader::selectDsc(const StructureValue &hdr, const QHash<QString, int> &clockBits)
{
    ...
    if (!m_schema || !m_tc || !m_tc->packetHeaderFieldClass)
        return Utils::ResultOk;
    const RoleField id = findUIntRole(m_tc->packetHeaderFieldClass.get(), hdr,
                                      UIntRole::DataStreamClassId, clockBits);
    if (!id.found)
        return Utils::ResultOk;
    ...
    return Utils::ResultError(
        u"No data stream class with id %1 for this packet"_s.arg(id.value));
}
...
std::optional<quint64> PacketReader::resolveFromRoots(const FieldLocation &loc) const
{
    ...
    switch (loc.origin) {
    case FieldLocation::Origin::PacketHeader:               root = &m_pktHeader;          break;
    case FieldLocation::Origin::PacketContext:              root = &m_pktContext;         break;
    case FieldLocation::Origin::EventRecordHeader:          root = &m_evHeader;           break;
    case FieldLocation::Origin::EventRecordCommonContext:   root = &m_evCommonContext;    break;
    case FieldLocation::Origin::EventRecordSpecificContext: root = &m_evSpecificContext;  break;
    case FieldLocation::Origin::EventRecordPayload:         return std::nullopt; // in-progress; scope stack handles it
    }
    if (!root)
        return std::nullopt;

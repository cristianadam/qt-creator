...
public:
    ...
    void asyncCopyFile(const Continuation<expected_str<void>> &cont, const FilePath &target) const;
    void asyncFileContents(const Continuation<const expected_str<QByteArray> &> &cont,
                           qint64 maxSize = -1,
                           qint64 offset = 0) const;
    void asyncWriteFileContents(const Continuation<expected_str<qint64>> &cont,

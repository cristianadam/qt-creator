...
static Result copy_data(struct archive *ar, struct archive *aw, std::function<bool()> isCancelled)
{
    int r;
    const void *buff;
    size_t size;
    ...
    while (!isCancelled()) {
        ...
        if (r == ARCHIVE_EOF)
            break;
        if (r < ARCHIVE_OK)
            return Result::Error(QString("%1").arg(QString::fromUtf8(archive_error_string(ar))));
        r = archive_write_data_block(aw, buff, size, offset);
        if (r < ARCHIVE_OK)
            return Result::Error(QString("%1").arg(QString::fromUtf8(archive_error_string(aw))));
    }
    return Result::Ok;
}
...
static Result unarchive(
    ...
    std::function<void(FilePath)> progress)
{
    ...
    if ((r = archive_read_open1(a.get())))
        return Result::Error(QString("%1").arg(QString::fromUtf8(archive_error_string(a.get()))));
    ...
    while (!isCancelled()) {
        ...
        if (r < ARCHIVE_OK) {
            return Result::Error(
                QString("%1").arg(QString::fromUtf8(archive_error_string(a.get()))));
        }

        if (progress)
            progress(FilePath::fromString(QString::fromUtf8(archive_entry_pathname_utf8(entry))));
        ...
        if (r < ARCHIVE_OK) {
            return Result::Error(
                QString("%1").arg(QString::fromUtf8(archive_error_string(ext.get()))));
        } else {
            ...
            if ((((stat->st_mode) & 0170000) == 0100000)) {
                r = copy_data(a.get(), ext.get(), isCancelled);
                if (r < ARCHIVE_OK) {
                    return Result::Error(
                        QString("%1").arg(QString::fromUtf8(archive_error_string(ext.get()))));
                }
...
Result unarchive(const Utils::FilePath &archive, const Utils::FilePath &destination)
{
    return unarchive(archive, destination, []() { return false; }, nullptr);
}
...
{
    promise.addResult(
        unarchive(archive, destination, [&promise]() { return promise.isCanceled(); }, callback));
}

...
{
    m_thread.setObjectName("Shell Thread (Inactive)");
    m_thread.start();
 ...
 * Will automatically defer to the internal thread
 */
DeviceShell::RunResult DeviceShell::outputForRunInShell(const CommandLine &cmd, const QByteArray &stdInData)
{
    if (QThread::currentThread() != &m_thread) {
        RunResult result;
        QMetaObject::invokeMethod(this, [this, &cmd, &stdInData] { return outputForRunInShellImpl(cmd, stdInData);}, Qt::BlockingQueuedConnection, &result);
        return result;
    }
...
bool DeviceShell::waitForStarted()
{
    if (QThread::currentThread() != &m_thread) {
        bool result;
        QMetaObject::invokeMethod(this, [this] { return waitForStarted();}, Qt::BlockingQueuedConnection, &result);
        return result;
    }
    return m_shellProcess->waitForStarted();
}

DeviceShell::RunResult DeviceShell::outputForRunInShellImpl(const CommandLine &cmd, const QByteArray &stdInData)
{
    ...
    qCInfo(deviceShellLog) << "Run command in shell:" << cmd.toUserOutput()
                           << "output size:" << result.stdOutput.size() << "exit code:" << result.exitCode;
    return result;
}
...
{
    if (QThread::currentThread() != &m_thread) {
        QMetaObject::invokeMethod(this, [this] { return closeShellProcess();}, Qt::BlockingQueuedConnection);
        return;
    }
    closeShellProcess();
}

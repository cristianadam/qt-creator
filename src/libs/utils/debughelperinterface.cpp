...
#include <utils/id.h>
// The translations came from utils originally
...
Q_LOGGING_CATEGORY(debugHelperLog, "qtc.debughelper", QtWarningMsg)
...
class DebugHelperInterfacePrivate : public QObject
{
    Q_OBJECT
public:
    DebugHelperInterfacePrivate(DebugHelperInterface *p)
        : q(p)
    {
        connect(&stubServer, &QLocalServer::newConnection, this, [this] {
            q->onNewStubConnection();
        });
    }
...
public:
    ...
    DebugHelperInterface *q;

    StubCreator *stubCreator;
};

DebugHelperInterface::DebugHelperInterface()
    : d(new DebugHelperInterfacePrivate(this))
{}
...
void DebugHelperInterface::setStubCreator(StubCreator *creator)
{
    d->stubCreator = creator;
}

int DebugHelperInterface::inferiorProcessId() const
{
    return d->inferiorProcessId;
}

int DebugHelperInterface::inferiorThreadId() const
{
...
void DebugHelperInterface::onNewStubConnection()
{
    d->stubConnectTimeoutTimer.reset();

    d->stubSocket = d->stubServer.nextPendingConnection();
    QObject::connect(d->stubSocket, &QIODevice::readyRead, this, [this] { onStubReadyRead(); });

    if (HostOsInfo::isAnyUnixHost()) {
        connect(d->stubSocket, &QLocalSocket::disconnected, this, [this] { onStubExited(); });
    }
}

void DebugHelperInterface::onStubExited()
{
...
void DebugHelperInterface::onStubReadyRead()
{
    while (d->stubSocket->canReadLine()) {
...
expected_str<void> DebugHelperInterface::startStubServer()
{
...
void DebugHelperInterface::shutdownStubServer()
{
    if (d->stubSocket) {
...
void DebugHelperInterface::emitError(QProcess::ProcessError error, const QString &errorString)
{
...
void DebugHelperInterface::emitFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
...
bool DebugHelperInterface::isRunning() const
{
    return (d->stubSocket && d->stubSocket->isOpen());
}

void DebugHelperInterface::cleanupAfterStartFailure(const QString &errorMessage)
{
    shutdownStubServer();
...
void DebugHelperInterface::sendCommand(char c)
{
...
void DebugHelperInterface::killInferiorProcess()
{
    sendCommand('k');
    d->stubSocket->waitForReadyRead();
}

void DebugHelperInterface::killStubProcess()
{
    if (!isRunning())
        return;
...
void DebugHelperInterface::start()
{
    if (isRunning())
        return;
    ...
    FilePath stubPath = FilePath::fromUserInput(QCoreApplication::applicationDirPath())
    ...
    if (debugHelperLog().isDebugEnabled())
        cmd.addArg("-v");
    ...
    QMetaObject::invokeMethod(
        d->stubCreator,
        [cmd, this] { return d->stubCreator->startStubProcess(cmd, m_setup); },
...
qint64 DebugHelperInterface::write(const QByteArray &data)
{
    Q_UNUSED(data);
    QTC_CHECK(false);
    return -1;
}
void DebugHelperInterface::sendControlSignal(Utils::ControlSignal controlSignal)
{
...
#include "debughelperinterface.moc"

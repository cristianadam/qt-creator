...
#endif
{
    ...
    if (read || append) {
        QTC_EXPECT_OR(m_filePath.fileContents().and_then(
                          [this](const auto &contents) -> expected_str<QByteArray> {
...
{
    QTC_EXPECT_OR(m_filePath.copyFile(FilePath::fromString(newName)), return false);
    return true;
}

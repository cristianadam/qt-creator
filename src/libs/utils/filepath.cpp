...
{
    // FIXME: Performance
    QString res = m_path.mid(m_rootLen);
    const QChar slash = QLatin1Char('/');
    return res.mid(res.lastIndexOf(slash) + 1);
}
...
bool FilePath::startsWithDriveLetter() const
{
    if (needsDevice() || !HostOsInfo::isWindowsHost())
        return false;
    return m_rootLen >= 2 && m_path.at(0).isLetter() && m_path.at(1) == ':';
}
...
FilePath FilePath::resolvePath(const FilePath &tail) const
{
    if (tail.m_rootLen > 0)
        return tail;
    return pathAppended(tail.m_path.mid(tail.m_rootLen));
}

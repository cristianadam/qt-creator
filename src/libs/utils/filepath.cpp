...
bool FilePath::isSameFile(const FilePath &other) const
{
    ...
    if (pathView().compare(other.pathView(), Qt::CaseSensitive) == 0)
        return true;
...
{
    const FilePath fromComspec = withNewPath(FilePath::fromUserInput(env.value("COMSPEC")).path());

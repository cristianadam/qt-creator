...
bool Unarchiver::setSourceFile(const FilePath &sourceFile)
{
    const auto data = sourceAndCommand(sourceFile);
    if (data)
        m_sourceAndCommand = *data;
    else
        m_sourceAndCommand = {};
    return bool(m_sourceAndCommand);
}

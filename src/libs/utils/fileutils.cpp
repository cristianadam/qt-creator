...
bool FileReader::fetch(const FilePath &filePath, QIODevice::OpenMode mode)
{
    ...
    // TODO: Always use filePath.fileContents() here.
    if (filePath.needsDevice()) {
        const auto contents = filePath.fileContents();
        if (!contents) {
            m_errorString = contents.error(); //tr("Cannot read %1").arg(filePath.toUserOutput());
            return false;
        }
        m_data = *contents;
        return true;
...
bool FileSaver::finalize()
{
    if (m_filePath.needsDevice()) {
        ...
        const auto res = m_filePath.writeFileContents(data);
        m_file->remove();
        m_file.reset();
...
bool FileUtils::copyIfDifferent(const FilePath &srcFilePath, const FilePath &tgtFilePath)
{
    ...
    if (tgtFilePath.exists()) {
        ...
        if (srcModified == tgtModified) {
            ...
            const auto srcContents = srcFilePath.fileContents();
            const auto tgtContents = srcFilePath.fileContents();
    ...
    QTC_TRY(srcFilePath.copyFile(tgtFilePath), return false);
    return true;
}

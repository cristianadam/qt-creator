    ...
    : d(new Internal::WrappedProcessInterfacePrivate(this))
{
    ...
    connect(&d->m_process, &Process::done, this, [this] {
        ...
        if (d->m_remotePID == 0 && !d->m_hasReceivedFirstOutput) {
            resultData.m_error = Utils::FailedToStart;
...
WrappedProcessInterface::~WrappedProcessInterface()
{
    if (d->m_process.state() == Utils::Running)
...
void WrappedProcessInterface::start()
{
    ...
    d->m_process.setProcessChannelMode(Utils::ProcessChannelMode::SeparateChannels);
    ...
    d->m_forwardStdout = m_setup.m_processChannelMode == Utils::ForwardedChannels
                         || m_setup.m_processChannelMode == Utils::ForwardedOutputChannel;
    d->m_forwardStderr = m_setup.m_processChannelMode == Utils::ForwardedChannels
                         || m_setup.m_processChannelMode == Utils::ForwardedErrorChannel;
    ...
    if (!fullCommandLine) {
        emit done(ProcessResultData{
            -1,
            Utils::ProcessExitStatus::CrashExit,
            Utils::ProcessError::FailedToStart,

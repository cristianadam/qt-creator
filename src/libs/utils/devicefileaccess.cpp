                                                          ...
                                                          const FilePath &target) const
{
    if (QFile::copy(filePath.path(), target.path()))
        return {};
    return unexpected<QString>(QString("Failed to copy file \"%1\" to \"%2\".")
                                   .arg(filePath.toUserOutput())
                                                                    ...
                                                                    qint64 offset) const
{
    ...
    QFile f(path);
    if (!f.exists())
        return unexpected<QString>(QString("File \"%1\" does not exist").arg(path));

    if (!f.open(QFile::ReadOnly))
        return unexpected<QString>(QString("Could not open File \"%1\"").arg(path));
                                                                     ...
                                                                     qint64 offset) const
{
    ...
    QTC_ASSERT(file.open(QFile::WriteOnly | QFile::Truncate),
               return unexpected<QString>("Could not open file \"" + filePath.toUserOutput()
                                          + "\" for writing"));
    if (offset != 0)
        file.seek(offset);
                                                       ...
                                                       const FilePath &target) const
{
    ...
    if (result.exitCode != 0) {
        return unexpected<QString>(QString("Failed to copy file %1 to %2: %3")
                                       .arg(filePath.toUserOutput())
                                       .arg(target.toUserOutput())
                                                                 ...
                                                                 qint64 offset) const
{
    ...
    if (r.exitCode != 0)
        return unexpected<QString>(QString("Failed reading file %1: %2")
                                       .arg(filePath.toUserOutput())
                                                                  ...
                                                                  qint64 offset) const
{
    ...
    if (result.exitCode != 0) {
        return unexpected<QString>(QString("Failed writing file %1: %2")
                                       .arg(filePath.toUserOutput())
                                       .arg(QString::fromUtf8(result.stdErr)));
    }
    return data.size();

...
private:
    ...
    void add(const QString &call, QStringList *to)
    {
        // A call carries markup of its own: a substitution stands for
        // the name of the command, and a reference for what it points
        // at.
        const QString text = RstLang::inlineToPlainText(call, _options).trimmed();
...
QStringList termNames(const QString &term, const RstLang::MarkdownOptions &options)
{
    ...
    if (!literals.isEmpty())
        return literals;
    return {RstLang::inlineToMarkdown(term, options)};
}
...
{
    RstLang::MarkdownOptions options;
    options.substitutions = rst ? rst->substitutionTexts() : QHash<QString, QString>();
    options.defaultLanguage = QStringLiteral("cmake");
    return RstLang::briefMarkdown(node, options);
}
...
QStringList Documentation::signatures() const
{
    ...
    Signatures signatures(rst->source(), name, options);
...
QList<ArgumentDoc> Documentation::arguments() const
{
    ...
    Arguments arguments(rst->source(), name, options);
    if (RstLang::DirectiveAST *directive = node->asDirective())

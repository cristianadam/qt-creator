    ...
    Process &process, const InstanceConfig &instanceConfig, const QString &context)
{
    ...
    QObject::connect(
        &process, &Process::textOnStandardOutput, [instanceConfig, context](const QString &text) {
            if (context.isEmpty())
                instanceConfig.logFunction(text.trimmed());
            else
        ...
        execInContainerTask(
            ...
            [containerDetails,
             runningDetails](const Process &process, DoneWith doneWith) -> DoneResult {
                ...
                if (!user) {
                    qCWarning(devcontainerlog)
                        << "Failed to parse user from passwd line:" << output;
                    return DoneResult::Error;
                }
    ...
    const CommandLine &prefix = {})
{
    ...
    auto logExecution = [instanceConfig, hookName]() {
    ...
    if (std::holds_alternative<QString>(*command)) {
        return Group{
            Sync(logExecution),
            singleCommandLifecycleRecipe(instanceConfig, std::get<QString>(*command), prefix)};
    }

    if (std::holds_alternative<QStringList>(*command)) {
        return Group{
            Sync(logExecution),
            singleCommandLifecycleRecipe(instanceConfig, std::get<QStringList>(*command), prefix)};
    }

    auto map = std::get<std::map<QString, std::variant<QString, QStringList>>>(*command);

    GroupItems commands;
    for (const auto &[key, value] : map)
        commands.push_back(singleCommandLifecycleRecipe(instanceConfig, value, prefix, key));

    return Group{Sync(logExecution), Group{parallelIdealThreadCountLimit, commands}};
}

...
{
    // Generate a unique ID for the devcontainer instance based on the workspace folder and config file path.
    const auto workspace = workspaceFolder.toUrlishString().toUtf8();
    const auto config = configFilePath.toUrlishString().toUtf8();
    ...
    if (it.hasNext()) {
        const QMap<QString, std::function<QString(QStringList)>> replacers = {
            {"localWorkspaceFolder",
             [this](const QStringList &) { return workspaceFolder.toUrlishString(); }},
            {"localWorkspaceFolderBasename",
             [this](const QStringList &) { return workspaceFolder.fileName(); }},
            {"containerWorkspaceFolder",
             [this](const QStringList &) { return containerWorkspaceFolder.toUrlishString(); }},
...
public:
    ...
    Result<CommandLine> wrapCommmandLine(
        const ProcessSetupData &setupData, QString markerTemplate) const override
    {

...
// The cell size a surface without a view in front of it lays images out for
...
struct TerminalSurfacePrivate
{
    ...
    const VTermScreenCell &blankCell()
    {
        if (!m_blankValid) {
            ...
            m_blank.chars[0] = 0;
            m_blank.width = 1;
            // The cell it is taken from may be part of a link or an image, the
            // empty space it pads the scrollback with is neither.
            m_blank.uri = 0;
            m_blank.image = 0;
            m_blankValid = true;
    ...
    {
        m_lastImageId = m_lastImageId % ImageCell::maxId + 1;

        if (m_images.contains(m_lastImageId)) {
            m_imageBytes -= m_images.take(m_lastImageId).image.sizeInBytes();
            m_imageOrder.removeOne(m_lastImageId);
        }
...
QSizeF TerminalSurface::cellSize() const
{
    return d->m_cellSize;
}

...
static bool shouldCall(CallDoneIf doneOn, bool success)
{
    if (success)
        return doneOn != CallDoneIf::Error;
    return doneOn != CallDoneIf::Success;
}

void TaskNode::invokeDoneHandler(bool success)
{
    if (m_taskHandler.m_doneHandler && shouldCall(m_taskHandler.m_doneOn, success))

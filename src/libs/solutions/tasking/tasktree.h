...
public:
    ...
    struct TaskHandler {
        ...
        CallDoneIf m_doneOn = CallDoneIf::SuccessOrError;
    };
...
public:
    ...
    template <typename Handler>
    static GroupItem onGroupDone(Handler &&handler, CallDoneIf doneOn = CallDoneIf::SuccessOrError) {
        return groupHandler({{}, wrapGroupDone(std::forward<Handler>(handler)), doneOn});
    }
...
static GroupItem onGroupDone(Handler &&handler, CallDoneIf doneOn = CallDoneIf::SuccessOrError)
{
    return Group::onGroupDone(std::forward<Handler>(handler), doneOn);
}
...
public:
    ...
    CustomTask(SetupHandler &&setup = SetupFunction(), DoneHandler &&done = DoneFunction(),
               CallDoneIf doneOn = CallDoneIf::SuccessOrError)
        : GroupItem({&createAdapter, wrapSetup(std::forward<SetupHandler>(setup)),
                     wrapDone(std::forward<DoneHandler>(done)), doneOn})
    {}

...
*/
    ...
    auto r = spec->readMetaData(spec->d->loader->metaData());
    if (!r)
    ...
    auto r = spec->readMetaData(plugin.metaData());
    if (!r)
        return make_unexpected(r.error());
...
bool PluginSpec::resolveDependencies(const QVector<PluginSpec *> &specs)
{
    ...
    for (const PluginDependency &dependency : deps) {
    ...
    setDependencySpecs(resolvedDependencies);
    setState(PluginSpecImpl::Resolved);
...
void PluginSpec::setDependencySpecs(const QHash<PluginDependency, PluginSpec *> &dependencySpecs)
{
    d->dependencySpecs = dependencySpecs;
}

...
void WindowsDeviceDetectionTest::testDetectToolchainsAndCreateKit()
{
    ...
    // Build the device through its factory (as the GUI does) so its device tool aspects (ninja,
    // ...) are initialized; the build-tool detection relies on them. Register it so device-rooted
    // process/file routing resolves to it.
    IDeviceFactory *factory = IDeviceFactory::find(Remote::Constants::GenericWindowsOsType);
    QVERIFY2(factory, "No Windows device factory is registered.");
    const IDevice::Ptr device = factory->construct();
    QVERIFY2(device, "The Windows device factory produced no device.");
    ...
    detectAndRegisterToolchainsForTest(std::dynamic_pointer_cast<WindowsDevice>(device));

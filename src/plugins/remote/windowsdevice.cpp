...
static QString killByPathScript(const FilePath &path)
{
    return "Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.Path -ieq "
           + psPath(path) + " } | Stop-Process -Force";
}
...
void WindowsProcessInterface::killRemoteApplication()
{
    ...
    QObject::connect(killer, &Process::done, killer, &QObject::deleteLater);
    killer->setCommand(
        {m_device->filePath("powershell.exe"),
...
QString WindowsProcessInterface::buildInteractiveRunRemoteCommand()
{
    ...
    script += "    Set-Content -Path $note -Value ('qtc: " + sessionUser + " is not logged on to "
              "the device, so the application runs without a desktop and a window of it would "
              "not be visible.')\n";
                                                ...
                                                const std::function<void()> &errorExitHandler)
{
    ...
    // A reconnect reuses the binary of the previous one instead of adding another, as long as
    // it is still there and still starts.
    if (!previousBridge.isEmpty() && previousBridge.exists()) {

...
{
public:
    SshConnectionHandler() { setObjectName("SshConnectionHandler"); }
    ...
    SshParameters displaylessSshParameters() const;
    ...
    SshConnectionHandle *connectionHandle, const SshParameters &sshParameters)
{
    ...
    const Qt::ConnectionType connectionType = QThread::currentThread() == guard->thread()
                                                  ? Qt::DirectConnection // should never happen
...
SshParameters SshConnectionHandler::displaylessSshParameters() const
{
    QMutexLocker locker(&m_mutex);
    return m_displaylessSshParameters;
}

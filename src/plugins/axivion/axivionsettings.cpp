...
static PathMapping showPathMappingsDialog(const PathMapping &suggested)
{
    ...
    QVBoxLayout *layout = new QVBoxLayout;
    auto label = new QLabel(Tr::tr("Configure a valid path mapping for '%1' to be able to open "
...
bool handleMissingPathMapping(const FilePath &missingPath, const QString &projectName)
{
    ...
    mbox.setText(Tr::tr("No valid path mapping for '%1' configured.").arg(missingPath.path()));
    mbox.setInformativeText(Tr::tr("To be able to open files for this project you need to specify "
                                   "a valid path mapping or select a matching local file."));
    QPushButton *filechooser = new QPushButton(Tr::tr("Select matching file..."));
    mbox.addButton(filechooser, QMessageBox::AcceptRole);
    QPushButton *manual = new QPushButton(Tr::tr("Set up manually..."), &mbox);
    ...
    if (clicked == filechooser) {
        ...
        // else manually set up has been clicked
        ProjectExplorer::Project *startupProj = ProjectExplorer::ProjectManager::startupProject();
        const FilePath computedPath = startupProj ? findFileForIssuePath(missingPath) : FilePath{};

...
void SquishTools::startSquishServer(Request request)
{
    if (m_shutdownInitiated)
        return;
    const SquishPerspective::PerspectiveMode runnerMode = m_perspective.perspectiveMode();
    QTC_ASSERT(runnerMode != SquishPerspective::NoMode, return);
    m_request = request;
    ...
    if (runnerMode == SquishPerspective::Running) {
        if (toolsSettings.minimizeIDE)
...
void SquishTools::handlePrompt(const QString &fileName, int line, int column)
{
    ...
    default:
        if (line != -1 && column != -1) {
            ...
            if (m_perspective.perspectiveMode() == SquishPerspective::Interrupted && !m_requestVarsTimer) {
...
{
    QTC_ASSERT(m_perspective.perspectiveMode() == SquishPerspective::Interrupted, return);

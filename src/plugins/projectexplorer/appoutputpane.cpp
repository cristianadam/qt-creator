...
void AppOutputPane::updateFilter()
{
    ...
    if (RunControlTab * const tab = currentTab()) {
        ...
        if (tab->runControl && filterText() != tab->lastReportedFilterText) {
            tab->lastReportedFilterText = filterText();
...
{
    RunControlTab * const ct = currentTab();
    if (!ct || ct->runControl != runControl)
        return;
    ct->lastReportedFilterText = text;

...
{
    return Internal::isPluginRunning(id);
}
...
{
    QTC_CHECK(id.isLower());

    return Utils::anyOf(
        ExtensionSystem::PluginManager::plugins(), [id](const ExtensionSystem::PluginSpec *s) {
            return s->state() == ExtensionSystem::PluginSpec::Running && s->id() == id;
        });
}

...
public:
    ...
    void doRun() final
    {
        m_source.asyncCopyFile([this](const expected_str<void> &cont) {
            if (!cont) {
...
public:
    CopyFileStep(BuildStepList *bsl, Id id)
        : CopyStepBase(bsl, id)
     {
        ...
        m_targetAspect->setExpectedKind(PathChooser::File);
...
{
    registerStep<CopyFileStep>("ProjectExplorer.CopyStep");

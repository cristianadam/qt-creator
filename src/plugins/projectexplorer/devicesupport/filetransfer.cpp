...
{
    emit done({0, Utils::NormalExit, Utils::FailedToStart, errorString});
}
...
{
    emitDone({0, Utils::NormalExit, Utils::FailedToStart, errorString});
}
...
static void setupTransfer(FileTransfer *transfer, QTaskInterface *iface)
{
    QObject::connect(transfer, &FileTransfer::done, iface, [iface](const ProcessResultData &result) {
        const bool success = result.m_exitStatus == Utils::NormalExit
                             && result.m_error == Utils::UnknownError
                             && result.m_exitCode == 0;

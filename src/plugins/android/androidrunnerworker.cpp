...
static ExecutableItem jdbMonitorRecipe(const Storage<RunnerStorage> &storage)
{
    ...
    const auto onSetup = [storage, startJdbBarrier, settledJdbBarrier] {
        ...
        QBarrier *start = startJdbBarrier.activeStorage();
        QBarrier *settled = settledJdbBarrier.activeStorage();
        // started is emitted from inside this tree's own handlers: a
        // synchronous advance() there re-enters the running tree.
        const auto queuedAdvance = [](auto *barrier) {
            return [barrier] {
                QMetaObject::invokeMethod(barrier, [barrier] { barrier->advance(); },
                                          Qt::QueuedConnection);
            };
        };
        if (!setJdbCallbacksForLogcat(storage->m_glue->runControl(),
                queuedAdvance(start), queuedAdvance(settled))) {
            // No listener: fail visibly instead of hanging at "Waiting for Debugger".
            storage->m_glue->runControl()->postMessage(
                Tr::tr("Logcat stream unavailable; cannot monitor the JDB handshake."),
                Utils::ErrorMessageFormat);
    ...
    const auto onDone = [storage] {
        if (storage->m_useCppDebugger)
            clearJdbCallbacksForLogcat(storage->m_glue->runControl());
    };

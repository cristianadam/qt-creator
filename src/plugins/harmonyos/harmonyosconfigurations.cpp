...
// The ABI such a mislabeled version has, which it does not know itself: the installed Qt
// reports none, because the metadata it would come from is empty. Take the architecture from
// the Qt libraries and pair it with the only OS they can be for.

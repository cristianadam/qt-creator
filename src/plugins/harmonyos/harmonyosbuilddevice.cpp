...
public:
    ...
    // DesktopDevice detects its tools here, which for a restored device happens while the
    // settings are still being read, and no kit can be created then. Public because the
    // device this plugin makes itself is not made by IDeviceFactory, which would call it.
...
#ifdef Q_OS_OHOS
    setConstructionFunction([] { return HarmonyOsThisDevice::create(); });
#else

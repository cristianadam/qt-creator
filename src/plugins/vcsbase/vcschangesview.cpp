    ...
    QVariant data(int column, int role) const final
    {
        ...
        if (role == Qt::DisplayRole) {
            ...
            case Section::Staged:
                ...
                break;
            case Section::Unmerged:
                title = Tr::tr("Merge Changes");
                break;
...
private:
    ...
    void documentSaved(const FilePath &filePath);
    ...
    , m_model(new TreeModel<>(this))
{
    ...
    connect(EditorManager::instance(), &EditorManager::saved,
        ...
        if (option != IDocument::SaveOption::AutoSave)
            documentSaved(document->filePath());
    });
...
void ChangesView::rebuildProjectTree()
{
    ...
    for (Project *project : ProjectManager::projects()) {
        ...
        qCDebug(log) << "rebuildProjectTree:" << project->displayName()
                     << project->projectDirectory() << "vc:" << (vc ? vc->displayName() : "-")
                     << "topLevel:" << topLevel
...
void ChangesView::updateRepositoryItem(RepositoryItem *item)
{
    ...
    if (vc->supportsStaging()) {
        for (const Section section : {Section::Unmerged, Section::Staged, Section::Changes}) {
...
void ChangesView::documentSaved(const FilePath &filePath)
{
    for (const RepositoryEntry &entry : std::as_const(m_repositories)) {
        if (!entry.vc->supportsAutomaticStatusUpdates()
            || !filePath.isChildOf(entry.repository)) {
            continue;
        }
        if (isVisible())
            entry.vc->requestRepositoryStatus(entry.repository);
...
void ChangesView::requestAutomaticRefresh()
{
    for (const RepositoryEntry &entry : std::as_const(m_repositories)) {
        if (!entry.vc->supportsAutomaticStatusUpdates())
            continue;

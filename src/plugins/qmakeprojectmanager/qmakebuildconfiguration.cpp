    ...
    : BuildConfiguration(target, id)
{
    ...
    runSystemFuctions.setSettingsKey("RunSystemFunction");
    runSystemFuctions.setDisplayStyle(SelectionAspect::DisplayStyle::ComboBox);
    runSystemFuctions.setDisplayName(Tr::tr("qmake system() behavior when parsing:"));
    runSystemFuctions.addOption(Tr::tr("Run"));
    runSystemFuctions.addOption(Tr::tr("Ignore"));
    runSystemFuctions.addOption(Tr::tr("Use global setting"));
    runSystemFuctions.setDefaultValue(2);
}
...
{
    const int sel = runSystemFuctions();
    if (sel == 0)
        return true;
    if (sel == 1)
        return false;

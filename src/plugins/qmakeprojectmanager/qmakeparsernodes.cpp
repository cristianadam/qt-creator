...
bool QmakePriFile::folderChanged(const QString &changedFolder, const QSet<FilePath> &newFiles)
{
    ...
    for (const FilePath &file : qAsConst(removedFiles)) {
...
bool QmakePriFile::addFiles(const FilePaths &filePaths, FilePaths *notAdded)
{
    ...
    FilePaths failedFiles;
    const auto types = typeFileMap.keys();
...
{
    QByteArray result;
    const auto defs = variableValue(Variable::Defines);
                                           ...
                                           const FilePath &buildDir, const QString &projectDir)
{
    ...
    const auto flagList = reader->values(QLatin1String("QMAKE_CXXFLAGS"));
    ...
    const auto sourceList = reader->fixifiedValues(QLatin1String("INCLUDEPATH"), projectDir,
                                                   buildDir.toString(), false);
    for (const ProFileEvaluator::SourceFile &el : sourceList) {
...
{
    QStringList result;
    const auto values = reader->values(QLatin1String("LIBS"));

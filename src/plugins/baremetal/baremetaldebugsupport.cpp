...
public:
    BareMetalDebugSupportFactory()
    {
        setProducer([](RunControl *runControl) {
            ...
            QObject::connect(runControl, &RunControl::aboutToStart, debugger, [runControl, debugger] {
                const auto dev = std::static_pointer_cast<const BareMetalDevice>(runControl->device());
                QTC_ASSERT(dev, debugger->reportFailure(); return);
                IDebugServerProvider *p = DebugServerProviderManager::findProvider(
                    dev->debugServerProviderId());
                QTC_ASSERT(p, debugger->reportFailure(); return);

                QString errorMessage;
                if (!p->aboutToRun(debugger, errorMessage))
                    debugger->reportFailure(errorMessage);
            });

    ...
    Tasks validate(const Kit *k) const override
    {
        ...
        if (path.isEmpty()) {
            ...
            if (!venvIsUsable(path)) {
                result << BuildSystemTask(
                    Task::Warning,
                    Tr::tr("Python %1 does not contain a usable venv. venv is a recommended way to "
                           "isolate a development environment for a project from the globally "
                           "installed python.")
                        .arg(path.toUserOutput()));
            }
        }

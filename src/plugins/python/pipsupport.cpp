...
Group pipInstallerTask(const PipInstallerData &data, const std::function<void(bool)> &doneHandler)
{
    ...
    const auto onDone = [packagesDisplayName, doneHandler](const Process &process) {
    ...
    return {
        ProcessTask(onSetup, onDone, CallDone::OnSuccess | CallDone::OnError)

...
static expected_str<void> loadKonsoleColorScheme(const FilePath &path)
{
    ...
    // clang-format off
    const QList<QPair<QStringView, ColorAspect *>> colorKeys = {
        qMakePair(u"Background/Color", &TerminalSettings::instance().backgroundColor),
        qMakePair(u"Foreground/Color", &TerminalSettings::instance().foregroundColor),

        qMakePair(u"Color0/Color", &TerminalSettings::instance().colors[0]),
        qMakePair(u"Color0Intense/Color", &TerminalSettings::instance().colors[8]),

        qMakePair(u"Color1/Color", &TerminalSettings::instance().colors[1]),
        qMakePair(u"Color1Intense/Color", &TerminalSettings::instance().colors[9]),

        qMakePair(u"Color2/Color", &TerminalSettings::instance().colors[2]),
        qMakePair(u"Color2Intense/Color", &TerminalSettings::instance().colors[10]),

        qMakePair(u"Color3/Color", &TerminalSettings::instance().colors[3]),
        qMakePair(u"Color3Intense/Color", &TerminalSettings::instance().colors[11]),

        qMakePair(u"Color4/Color", &TerminalSettings::instance().colors[4]),
        qMakePair(u"Color4Intense/Color", &TerminalSettings::instance().colors[12]),

        qMakePair(u"Color5/Color", &TerminalSettings::instance().colors[5]),
        qMakePair(u"Color5Intense/Color", &TerminalSettings::instance().colors[13]),

        qMakePair(u"Color6/Color", &TerminalSettings::instance().colors[6]),
        qMakePair(u"Color6Intense/Color", &TerminalSettings::instance().colors[14]),

        qMakePair(u"Color7/Color", &TerminalSettings::instance().colors[7]),
        qMakePair(u"Color7Intense/Color", &TerminalSettings::instance().colors[15])
    };
    // clang-format on
...
static expected_str<void> loadXFCE4ColorScheme(const FilePath &path)
{
    ...
    // clang-format off
    const QList<QPair<QStringView, ColorAspect *>> colorKeys = {
        qMakePair(u"Scheme/ColorBackground", &TerminalSettings::instance().backgroundColor),
        qMakePair(u"Scheme/ColorForeground", &TerminalSettings::instance().foregroundColor),
    };
    // clang-format on

    for (const auto &colorKey : colorKeys) {
        if (settings.contains(colorKey.first)) {
            colorKey.second->setVolatileValue(
                QColor::fromString(settings.value(colorKey.first).toString()));
        }
    }

    QStringList colors = settings.value(u"Scheme/ColorPalette").toStringList();
    int i = 0;
    for (const auto &color : colors) {
        TerminalSettings::instance().colors[i++].setVolatileValue(QColor::fromString(color));
    }

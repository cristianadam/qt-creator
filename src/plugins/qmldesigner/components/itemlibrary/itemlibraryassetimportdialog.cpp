...
void ItemLibraryAssetImportDialog::startPreview()
{
    ...
    // Preview is done view custom QML file added into the temporary folder of the preview
    QString previewQml =
R"(
import QtQuick
import QtQuick3D
...
Rectangle {
    ...
    // TODO: Remove previewfile once import is finalized, before copying files

    if (!m_previewFile.exists()) {
        // TODO: Proper error handling
        qWarning() << __FUNCTION__ << "Failed to write preview file";
        return;
    }
    ...
    if (!m_rewriterView->errors().isEmpty()) {
        ...
        qWarning() << __FUNCTION__ << "Rewriter errors!";
        return;
    }
    ...
    auto crashCallback = [] {
        ...
        qWarning() << __FUNCTION__ << "Puppet crashed!";
    };
...
void ItemLibraryAssetImportDialog::onImportReadyForPreview(const QString &path, const QString &compName)
{
    ...
    addInfo(tr("Import preview ready for %1").arg(compName));
}

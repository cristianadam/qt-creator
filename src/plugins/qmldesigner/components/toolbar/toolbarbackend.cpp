    ...
    : QObject(parent)
{
    ActionAddedInterface callback = [this](ActionInterface *interface) {
        if (interface->menuId() == m_actionId.toLatin1()) {
            ...
            qDebug() << "action subscribed " + m_actionId << tooltip();
            setupNotifier();
        }
    };
...
CrumbleBarModel::CrumbleBarModel(QObject *parent)
{
    connect(crumbleBar(), &CrumbleBar::pathChanged, this, &CrumbleBarModel::handleCrumblePathChanged);
}

int CrumbleBarModel::rowCount(const QModelIndex &parent) const
{
...
{
    beginResetModel();
    qDebug() << Q_FUNC_INFO << crumbleBar()->path();
    endResetModel();
}
...
void CrumbleBarModel::onCrumblePathElementClicked(int i)
{
    if (i < rowCount()) {
        ...
        qDebug() << Q_FUNC_INFO << info.displayName;

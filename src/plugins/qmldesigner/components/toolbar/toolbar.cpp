...
QString qmlSourcesStatusBarPath()
{
#ifdef SHARE_QML_PATH
    if (qEnvironmentVariableIsSet("LOAD_QML_FROM_SOURCE"))
        return QLatin1String(SHARE_QML_PATH) + "/statusbar";
#endif
    return Core::ICore::resourcePath("qmldesigner/statusbar").toString();
}
...
#ifdef SHARE_QML_PATH
    if (qEnvironmentVariableIsSet("LOAD_QML_FROM_SOURCE"))
        return QLatin1String(SHARE_QML_PATH) + "/toolbar";
#endif
    return Core::ICore::resourcePath("qmldesigner/toolbar").toString();
}
...
void ToolBar::createStatusBar()
{
    ...
    QString qmlFilePath = qmlSourcesStatusBarPath() + QStringLiteral("/Main.qml");
    QTC_ASSERT(QFileInfo::exists(qmlFilePath), return );

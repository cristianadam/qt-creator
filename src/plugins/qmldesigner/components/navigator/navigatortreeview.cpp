...
void NavigatorTreeView::mousePressEvent(QMouseEvent *event)
{
    ...
    // This is necessary as it is selected items that are dragged and the pressed item may not
    // be a selected item, e.g. when pressing on locked item, leading to unexpected drags.
    m_blockDragStart = !(model()->flags(indexAt(event->pos())) & Qt::ItemIsDragEnabled);

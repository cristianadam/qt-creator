                                               ...
                                               const QmlDesigner::ModelNode &node)
{
    ...
    if (node.hasAuxiliaryData(AuxiliaryDataType::Document, "locked"))
    ...
    for (auto &&grp : timeline.keyframeGroupsForTarget(node)) {
        if (grp.isValid()) {
            ...
            if (!curve.isEmpty()) {
                ...
                if (target.hasAuxiliaryData(AuxiliaryDataType::Document, "locked"))
                    propertyItem->setLocked(true);

                if (target.hasAuxiliaryData(AuxiliaryDataType::Document, "pinned"))
...
AnimationCurve CurveEditorModel::createDoubleCurve(const QmlDesigner::QmlTimelineKeyframeGroup &group)
{
    ...
    if (auto data = target.auxiliaryData(AuxiliaryDataType::Document, "unified"))
        str = data->toString();

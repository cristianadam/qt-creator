...
template<typename Tuple>
bool isType(const Tuple &types, const TypeName &first, const TypeName &second)
{
    return isType(types, first) && isType(types, second);
}
...
bool compareTypes(const NodeMetaInfo &sourceType, const NodeMetaInfo &targetType)
{
    if constexpr (useProjectStorage()) {
        ...
        const TypeName source = sourceType.typeName();
        const TypeName target = targetType.typeName();

        static constexpr auto variantTypes = std::make_tuple("alias", "unknown", "variant", "var");
        static constexpr auto numericTypes = std::make_tuple("double", "real", "int");
        static constexpr auto colorTypes = std::make_tuple("QColor", "color");
        static constexpr auto stringTypes = std::make_tuple("QString", "string");

        return isType(variantTypes, target) || isType(variantTypes, source)
               || (targetType == sourceType) || (isType(numericTypes, target, source))
               || (isType(colorTypes, target, source)) || (isType(stringTypes, target, source));
    }
}
} // namespace
    ...
    if (!m_dialog.isNull() && m_backendValueType) {
        ...
        if constexpr (useProjectStorage()) {
            targetString = " [" + (m_targetName.isEmpty() ? QString() : (m_targetName + ": "))
                           + QString::fromUtf8(m_backendValueType.typeName()) + "]";
        } else {

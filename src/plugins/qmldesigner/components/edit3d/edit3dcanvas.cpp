...
void Edit3DCanvas::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if (m_flyMode && e->globalPosition().toPoint() != m_hiddenCursorPos) {
        if (!m_flyModeFirstUpdate) {
            ...
            QPointF diff = m_isQDSTrusted ? (m_hiddenCursorPos - e->globalPosition().toPoint()) : (m_lastCursorPos - e->globalPosition().toPoint());

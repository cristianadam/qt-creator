                                            ...
                                            const QVariant &value)
{
    ...
    case AuxiliaryDataType::Document:
        if ((key.name == "invisible" || key.name == "locked") && hasInstanceForModelNode(node)) {
            NodeInstance instance = instanceForModelNode(node);
            if (value.isValid()) {
...
CreateSceneCommand NodeInstanceView::createCreateSceneCommand()
{
    ...
    for (const ModelNode &node : std::as_const(nodeList)) {
        ...
        if (node.isValid() && hasInstanceForModelNode(node)) {
            NodeInstance instance = instanceForModelNode(node);
            for (const auto &element : node.auxiliaryData(AuxiliaryDataType::NodeInstance)) {
    ...
    if (!qml3DNodes.isEmpty()) {
        ...
        for (const auto &node : qAsConst(qml3DNodes)) {
            if (rotationKeyframeTargets.contains(node))
                node.setAuxiliaryData(AuxiliaryDataType::NodeInstance, auxDataProp, true);
            else
                node.setAuxiliaryData(AuxiliaryDataType::NodeInstance, auxDataProp, false);
        }
    }
}

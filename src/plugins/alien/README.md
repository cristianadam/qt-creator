# VSIX extension infrastructure (prototype)

This plugin is the seed for running VSIX extensions inside Qt Creator.

Visual Studio Code is a trademark of Microsoft Corporation; this plugin is not
affiliated with, sponsored by or endorsed by Microsoft. It refers to VS Code
below only to say which extensions it is compatible with. In the user
interface the extensions are named after their package format, VSIX, since
that is what they are rather than whose they are.

## Using it

1. Enable the (experimental) Alien plugin and restart.
2. In *Preferences > VSIX Extensions*, tick **Enable VSIX extension
   support** and confirm the **Node.js path** (auto-detected from `PATH`).
3. Install extensions with **Install Extension (.vsix)...**, which
   unpacks a `.vsix` into the **Extensions directory** - by default Qt
   Creator's own `alien/extensions` under the user resource path. The setting
   can be pointed at another folder of extensions, bearing in mind that some
   are licensed for use with one editor only.
4. Extensions with a JavaScript entry point are activated in the Node host
   ...
   pane; **Rescan VSIX Extensions** re-reads the directory.
...
in the extensions directory (dotted keys, as extensions expect), e.g.:

    { "qt-qml.qmlls.customExePath": "/path/to/qmlls" }

## What a VSIX extension is
...
- Highlighting is KSyntaxHighlighting (Kate XML), *not* TextMate; extension
  grammars need a TextMate engine or conversion.
- Licensing: no registry is contacted today. A `.vsix` is a file the user
  already has, installing one only unpacks it, and the plugin has no network
  code at all - so no registry's terms bind Qt Creator here, and the download
  is the user's own act. Should browse-and-install arrive it should target
  **Open VSX**, not the Microsoft marketplace, whose terms limit that service
  to Visual Studio family products. Open VSX's own terms
  (`open-vsx.org/terms-of-use`, dated February 11, 2021) put no restriction on
  which client may download from it, and four of their clauses would then
  shape the UI:
  - Accessing the site *is* accepting those terms, and "you" covers whoever
    authorised the access - so a download belongs behind something the user
    asked for, with the terms in reach, not behind a background fetch.
  - Extensions stay under their own publishers' licenses, "including
    proprietary licenses", and complying with them is the user's
    responsibility. The registry launders nothing.
  - No rights to publisher logos or trademarks come with the content, which a
    UI showing extension icons would otherwise be relying on.
  - Content and its availability are "as is", may be removed or changed
    without notice, liability is capped at five euros, and the Foundation
    reserves the right to block an address - so cache, do not poll hard, and
    do not make a feature depend on the registry being up.
...
- `ExtensionRegistry` discovers installed extensions under the configured
  directory (default `alien/extensions` under the user resource path), each in
  its own subfolder with a `package.json`.

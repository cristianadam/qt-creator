...
public:
    ...
    Result<> initialize(const QStringList &arguments) final
    {
        ...
        const int dumpIndex = arguments.indexOf("-mcp-dump-tools");
        if (dumpIndex >= 0) {
            if (dumpIndex + 1 >= arguments.size())
                return ResultError(Tr::tr("-mcp-dump-tools requires a file argument."));
            const QString dumpPath = arguments.at(dumpIndex + 1);
            connect(ICore::instance(), &ICore::coreOpened, this, [dumpPath] {
                const bool written = dumpToolsToQdoc(dumpPath);
                // Exit non-zero when the dump failed: the regenerate target has to
                // fail with it, or a stale committed file passes as up to date.
                QMetaObject::invokeMethod(
                    qApp, [written] { qApp->exit(written ? 0 : 1); }, Qt::QueuedConnection);
            });
            return ResultOk;
        }
    ...
    // sorted by name, for inclusion in the documentation.
    static bool dumpToolsToQdoc(const QString &path)
    {
        QList<Schema::Tool> tools = ToolRegistry::registeredTools();
        Utils::sort(tools, [](const Schema::Tool &a, const Schema::Tool &b) {
            return a.name() < b.name();
        });
        // The header and note stay outside the "tools" snippet so they do not
        // render; the page pulls the snippet with "\include ... tools".
        QString out = "// Copyright (C) 2026 The Qt Company Ltd.\n"
                      "// SPDX-License-Identifier: LicenseRef-Qt-Commercial OR "
                      "GFDL-1.3-no-invariants-only\n\n"
                      "// This file is generated by \"qtcreator -mcp-dump-tools\".\n"
                      "// Do not edit; build the regenerate_mcp_tools_doc target instead.\n\n"
                      "//! [tools]\n"
                      "\\table\n"
                      "\\header\n"
                      "    \\li Tool\n"
                      "    \\li Description\n";
        const auto qdocText = [](QString s) {
            s.replace('\\', QLatin1String("\\\\")); // qdoc: literal backslash
            s.replace('\n', ' ');
            return s;
        };
        for (const Schema::Tool &tool : tools) {
            ...
            if (schema.properties() && !schema.properties()->isEmpty()) {
                const QStringList required = schema.required().value_or(QStringList());
                const QMap<QString, QJsonObject> &props = *schema.properties();
                out += "        \\list\n";
                for (auto it = props.constBegin(); it != props.constEnd(); ++it) {
                    QStringList notes;
                    const QString type = it.value().value("type").toString();
                    if (!type.isEmpty())
                        notes << type;
                    if (required.contains(it.key()))
                        notes << "required";
                    QString line = QString("            \\li \\c {%1}").arg(it.key());
                    if (!notes.isEmpty())
                        line += QString(" (%1)").arg(notes.join(", "));
                    const QString desc = qdocText(it.value().value("description").toString());
                    if (!desc.isEmpty())
                        line += " - " + desc;
                    out += line + '\n';
                }
        ...
        out += "\\endtable\n";
        out += "//! [tools]\n";
        const Result<qint64> result
            = Utils::FilePath::fromUserInput(path).writeFileContents(out.toUtf8());
        if (!result) {
            qWarning() << "mcp-dump-tools: cannot write" << path << ":" << result.error();
            return false;
        }
        return true;
    }

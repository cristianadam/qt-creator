...
public:
    ...
    static void dumpToolsToQdoc(const QString &path)
    {
        ...
        for (const Schema::Tool &tool : tools) {
            QString description = tool.description().value_or(QString());
            description.replace('\\', QLatin1String("\\\\")); // qdoc: literal backslash
            description.replace('\n', ' ');
            out += "\\row\n";
            out += QString("    \\li \\c {%1}\n").arg(tool.name());
            out += QString("    \\li %1\n").arg(description);

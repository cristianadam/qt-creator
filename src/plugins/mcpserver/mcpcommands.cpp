        ...
        Schema::Tool()
            ...
            .annotations(
                Schema::ToolAnnotations()
                    ...
                    .openWorldHint(false)
                    .readOnlyHint(false)),
        [](const Mcp::Schema::CallToolRequestParams &) -> Utils::Result<Server::TaskCallbacks> {
            auto startupProject = ProjectManager::instance()->startupProject();
            if (!startupProject) {
                qCDebug(mcpCommands) << "No startup project found, cannot build";
                return ResultError("No startup project found");
            }
            BuildManager::buildProjects({startupProject}, ConfigSelection::Active);

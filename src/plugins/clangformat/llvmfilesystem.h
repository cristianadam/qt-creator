...
public:
                                                                 ...
                                                                 bool IsVolatile = false) override
    {
        ...
        return llvm::MemoryBuffer::getMemBufferCopy(contents->data(), Name);
    }
...
public:
    ...
    llvm::ErrorOr<Status> status(const Twine &Path) override
    {
        ...
        Status
            result(Path, {0, 0}, modTime, 0, 0, fInfo.size(), type, llvm::sys::fs::perms::all_perms);
        return result;
        /*
        Status status;
        const auto info = path.filePathInfo();
        if (!info.fileFlags)
            return 
    ...
    // USED BY CLANG-FORMAT
    virtual llvm::ErrorOr<std::unique_ptr<File>> openFileForRead(const Twine &Path) override
    {

    ...
    , d(new DockerDevicePrivate(this))
{
    setFileAccess([this]() -> DeviceFileAccess * {
        if (auto fileAccess = d->m_fileAccess.readLocked()->get())
            return fileAccess;

        auto fileAccess = d->m_fileAccess.writeLocked();
        if (*fileAccess)
            return fileAccess->get();

        auto fAccess = std::make_unique<DockerDeviceFileAccess>(d);
        auto initResult = fAccess->init(rootPath().withNewPath("/tmp/_qtc_cmdbridge"));

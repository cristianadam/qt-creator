...
{
    emit q->logOutput(Tr::tr("Start removing auto-detected items associated with this docker image."));

    emit q->logOutput('\n' + Tr::tr("Removing kits..."));
    for (Kit *kit : KitManager::kits()) {
        if (kit->autoDetectionSource() == m_sharedId) {
            emit q->logOutput(Tr::tr("Removed \"%1\"").arg(kit->displayName()));
            KitManager::deregisterKit(kit);
        }
    };

    emit q->logOutput('\n' + Tr::tr("Removing Qt version entries..."));
    for (QtVersion *qtVersion : QtVersionManager::versions()) {
        if (qtVersion->detectionSource() == m_sharedId) {
            emit q->logOutput(Tr::tr("Removed \"%1\"").arg(qtVersion->displayName()));
            QtVersionManager::removeVersion(qtVersion);
        }
    };

    emit q->logOutput('\n' + Tr::tr("Removing toolchain entries..."));
    const Toolchains toolchains = ToolChainManager::toolchains();
    for (ToolChain *toolChain : toolchains) {
        if (toolChain && toolChain->detectionSource() == m_sharedId) {
            emit q->logOutput(Tr::tr("Removed \"%1\"").arg(toolChain->displayName()));
    ...
    emit q->logOutput('\n' + Tr::tr("Removal of previously auto-detected kit items finished.") + "\n\n");
}

void KitDetectorPrivate::listAutoDetected() const
{
    emit q->logOutput(Tr::tr("Start listing auto-detected items associated with this docker image."));
    ...
    emit q->logOutput('\n' + Tr::tr("Listing of previously auto-detected kit items finished.") + "\n\n");
}
...
QtVersions KitDetectorPrivate::autoDetectQtVersions() const
{
        ...
        if (QtVersion *qtVersion = QtVersionFactory::createQtVersionFromQMakePath(qmake,
            ...
            if (qtVersion->isValid()) {
                if (!Utils::anyOf(qtVersions,
                    ...
                    emit q->logOutput(
                        Tr::tr("Found \"%1\"").arg(qtVersion->qmakeFilePath().toUserOutput()));
                }
            }
        }
        return true;
    ...
    emit q->logOutput(Tr::tr("Searching for qmake executables..."));
    ...
    if (!error.isEmpty())
        emit q->logOutput(Tr::tr("Error: %1.").arg(error));
    if (qtVersions.isEmpty())
        emit q->logOutput(Tr::tr("No Qt installation found."));
    return qtVersions;
}
...
Toolchains KitDetectorPrivate::autoDetectToolChains()
{
    ...
    emit q->logOutput('\n' + Tr::tr("Searching toolchains..."));
    for (ToolChainFactory *factory : factories) {
        emit q->logOutput(Tr::tr("Searching toolchains of type %1").arg(factory->displayName()));
        ...
        for (ToolChain *toolChain : newToolChains) {
            emit q->logOutput(Tr::tr("Found \"%1\"").arg(toolChain->compilerCommand().toUserOutput()));
    ...
    emit q->logOutput(Tr::tr("%1 new toolchains found.").arg(allNewToolChains.size()));
...
void KitDetectorPrivate::autoDetect()
{
    ...
    emit q->logOutput(Tr::tr("Starting auto-detection. This will take a while..."));
    ...
    emit q->logOutput('\n' + Tr::tr("Registered kit %1").arg(kit->displayName()));

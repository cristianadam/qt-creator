...
public:
    ...
    std::unique_ptr<TaskTree> m_processTree;
...
void UpdateInfoPlugin::startCheckForUpdates()
{
    ...
    connect(futureProgress, &FutureProgress::canceled, this, [this] {
        stopCheckForUpdates();
    });
    ...
    const auto doCleanup = [this] {
        d->m_processTree.release()->deleteLater();
        checkForUpdatesStopped();
    };

    const OnSubTreeDone onTreeDone([this, fi, doCleanup]() mutable {
        fi.reportFinished();
        checkForUpdatesFinished();
        doCleanup();
    });
    const OnSubTreeError onTreeError([fi, doCleanup]() mutable {
        fi.reportCanceled(); // is used to indicate error
        fi.reportFinished();
        doCleanup();
    });

    QList<TaskBase> treeContent {parallel};
    ...
    treeContent << Process(setupUpdate, updateDone);
    ...
    if (d->m_settings.checkForQtVersions) {
        ...
        treeContent << Process(setupPackages, packagesDone);
    }
    treeContent << onTreeDone << onTreeError;

    d->m_processTree.reset(new TaskTree(Task{treeContent}));
    d->m_processTree->start();
}
...
{
    return d->m_processTree.get() != nullptr;
}

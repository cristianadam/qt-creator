    ...
    , m_title(Tr::tr("New Chat"))
{
    ...
    connect(m_controller, &AcpChatController::statusMessage,
            m_chatPanel, &ChatPanel::addStatusMessage);
    ...
    connect(m_controller, &AcpChatController::authenticationRequired,
        ...
        m_authMethodCombo->disconnect(this);
        m_authMethodCombo->clear();
        for (const Acp::AuthMethod &m : methods)
            m_authMethodCombo->addItem(m.name(), m.id());

        const auto updateDescription = [this, methods] {
            const int idx = m_authMethodCombo->currentIndex();
            if (idx >= 0 && idx < methods.size()) {
                const auto &desc = methods.at(idx).description();
                if (desc.has_value() && !desc->isEmpty()) {
                    m_authDescriptionLabel->setText(*desc);
                    m_authDescriptionLabel->show();
                    return;
                }
            }
            m_authDescriptionLabel->hide();
        ...
        updateDescription();
        connect(m_authMethodCombo, &QComboBox::currentIndexChanged,
    ...
    connect(m_controller, &AcpChatController::authenticationFailed,
        ...
        m_authErrorLabel->setText(Tr::tr("Authentication failed: %1").arg(error));
    ...
    connect(m_controller, &AcpChatController::sessionUpdate,
        ...
        if (kind == QLatin1String("agent_message_chunk")) {
            ...
            if (const auto *mu = update.get<CurrentModeUpdate>())
                m_chatPanel->addStatusMessage(Tr::tr("Mode changed: %1").arg(mu->currentModeId()));
    ...
    connect(m_controller, &AcpChatController::errorOccurred, this, [this](const QString &msg) {
        m_chatPanel->addStatusMessage(Tr::tr("Error: %1").arg(msg));
    });
}

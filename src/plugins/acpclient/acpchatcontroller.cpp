...
{
    const int version = result.value(QStringLiteral("protocolVersion")).toInt(-1);
    if (version == 1) {
        ...
        emit errorOccurred(Tr::tr("Unsupported ACP protocol version: %1")
                               .arg(result.value(QStringLiteral("protocolVersion"))
                                        .toVariant().toString()));
        disconnectFromServer();
    }
}
                                       ...
                                       const QJsonObject &rawCapabilities)
{
    ...
    m_agentName = info.title().value().value_or(info.name());
    m_agentVersion = info.version();

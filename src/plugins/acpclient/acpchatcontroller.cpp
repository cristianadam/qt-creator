...
void AcpChatController::connectToServer(const QString &serverId, const FilePath &workingDirectory)
{
    ...
    if (serverInfo.connectionType == AcpSettings::Stdio) {
        ...
        const FilePath command = cmdLine.executable().searchInPath();
    ...
    connect(m_transport, &AcpTransport::started, this, [this] {
        emit statusMessage(Tr::tr("Connected. Initializing..."));
    ...
    emit statusMessage(Tr::tr("Connecting..."));
    m_transport->start();
}
...
void AcpChatController::createNewSession()
{
    if (!m_client || !m_initialized) {
        emit statusMessage(Tr::tr("Connect to an agent first."));
        return;
    }

    emit statusMessage(Tr::tr("Creating new session..."));
    ...
    m_client->newSession(buildNewSessionRequest(),
        ...
        if (error) {
            ...
            emit statusMessage(Tr::tr("Session error: %1").arg(error->message()));
            return;
        }
        auto response = fromJson<NewSessionResponse>(QJsonValue(result));
        if (!response) {
            emit statusMessage(Tr::tr("Session error: invalid response"));
            return;
        }
        ...
        emit statusMessage(Tr::tr("Session ready. You can start chatting."));
...
void AcpChatController::sendPrompt(const QString &text)
{
    ...
    m_client->prompt(request, [this](const QJsonObject &result, const std::optional<Error> &error) {
        if (error) {
            emit statusMessage(Tr::tr("Prompt error: %1").arg(error->message()));
        } else {
...
void AcpChatController::cancelPrompt()
{
    ...
    emit statusMessage(Tr::tr("Cancelling..."));
}
...
void AcpChatController::authenticate(const QString &methodId)
{
    ...
    emit statusMessage(Tr::tr("Authenticating..."));
    ...
    m_client->authenticate(req, [this](const QJsonObject &result, const std::optional<Error> &error) {
        ...
        emit statusMessage(Tr::tr("Authentication successful."));
        createNewSession();
    });
}
...
void AcpChatController::setConfigOption(const QString &configId, const QString &value)
{
    ...
    m_client->setSessionConfigOption(req, [this](const QJsonObject &result, const std::optional<Error> &error) {
        if (error) {
            emit statusMessage(Tr::tr("Failed to set config option: %1").arg(error->message()));
            return;
        }
...
void AcpChatController::onStateChanged(int state)
{
    ...
    case AcpClientObject::State::Disconnected:
        emit statusMessage(Tr::tr("Disconnected"));
        m_initialized = false;
        if (m_connected) {
            m_connected = false;
            emit connectionStateChanged(false);
...
void AcpChatController::onInitializeResult(const InitializeResponse &response)
{
    ...
    if (m_agentName.isEmpty())
        emit statusMessage(Tr::tr("Connected"));
    else
        emit statusMessage(
            Tr::tr("Initialized (protocol v%1). Agent: %2 v%3")
                .arg(response.protocolVersion())
                .arg(m_agentName, m_agentVersion));

    m_initialized = true;

    m_authMethods = response.authMethods().value_or({});
    createNewSession();
}

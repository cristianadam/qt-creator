...
protected:
    void paintEvent(QPaintEvent *) override
    {
        ...
        Utils::StyleHelper::drawCardBg(&p, rect(),
                                       ...
                                       Utils::StyleHelper::SpacingTokens::RadiusM);
    }
};
    ...
    : QWidget(parent)
{
    ...
    m_progressIndicator = new Utils::ProgressIndicator(Utils::ProgressIndicatorSize::Small, toolbar);
    ...
    statusPal.setColor(QPalette::WindowText,
    ...
    auto *disconnectButton = new QPushButton(Tr::tr("Disconnect"));
...
void ChatPanel::updateConfigOptions(const QList<SessionConfigOption> &configOptions)
{
    for (const SessionConfigOption &option : configOptions) {
        ...
        if (!combo) {
            auto *label = new QLabel(option.name() + QLatin1Char(':'));
...
void ChatPanel::addStatusMessage(const QString &text)
{

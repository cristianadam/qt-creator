    ...
    : CollapsibleFrame(parent)
{
    ...
    const ToolCallStatus status
        = toolCall.status().value().value_or(ToolCallStatus::in_progress);
    ...
    if (const auto icon = iconForToolKind(toolCall.kind().value())) {
    ...
    m_titleLabel = new Utils::ElidingLabel(
        toolCall.title().value().value_or(QStringLiteral("Tool Call")), this);

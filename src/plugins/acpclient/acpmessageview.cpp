...
private:
    void renderOutput()
    {
        m_browser->setMarkdown(QStringLiteral("```\n%1\n```")
...
void AcpMessageView::updateToolCall(const ToolCallUpdate &update)
{
    ...
    if (!detail) {
        finishAgentMessage();
        const QString title = update.title().value().value_or(QStringLiteral("Tool Call"));
        auto *group = ensureToolCallGroup();
        group->trackStatus(update.toolCallId(),
                           update.status().value().value_or(ToolCallStatus::in_progress));
...
void AcpMessageView::updateTerminal(const TerminalUpdate &update)
{
    ...
    if (update.exitStatus().has_value())
        terminal->setExited(update.exitStatus()->exitCode().value());
}
                                           ...
                                           const RequestPermissionRequest &request)
{
    ...
    const QString title = toolCall.title().value().value_or(QString());
    ...
    if (!detail) {
        ...
        group->trackStatus(toolCallId, toolCall.status().value().value_or(ToolCallStatus::pending));
        if (!title.isEmpty())

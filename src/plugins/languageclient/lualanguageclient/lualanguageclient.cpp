...
public:
    ...
    LuaClientWrapper(const sol::table &options)
    {
        ...
        m_initOptionsCallback = addValue<QString>(
            ...
            [](const sol::protected_function_result &res) -> expected_str<QString> {
                if (res.get_type(0) == sol::type::table) {
                    QJsonValue json = ::Lua::LuaEngine::toJson(res.get<sol::table>());
                    return QString::fromUtf8(
                        QJsonDocument(json.toObject()).toJson(QJsonDocument::JsonFormat::Compact));
    ...
    void updateOptions()
    {
        ...
        if (m_initOptionsCallback) {
            auto result = m_initOptionsCallback(m_initializationOptions);
            if (!result)

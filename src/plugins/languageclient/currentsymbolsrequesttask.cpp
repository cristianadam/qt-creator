...
#include "currentsymbolsrequesttask.h"
...
void CurrentSymbolsRequestTask::start()
{
    QTC_ASSERT(!isRunning(), return);

    m_currentSymbolsData = {};
                                                              ...
                                                              const DocumentSymbolsResult &symbols)
    {
        ...
        m_currentSymbolsData = {filePath, pathMapper, symbols};
        clearConnections();
        emit done(true);
    };
    ...
    m_connections.append(connect(EditorManager::instance(), &EditorManager::currentEditorChanged,
                                 this, reportFailure));
    m_connections.append(connect(LanguageClientManager::instance(),
...
bool CurrentSymbolsRequestTask::isRunning() const
{
    return !m_connections.isEmpty();
}

void CurrentSymbolsRequestTask::clearConnections()
{
...
CurrentSymbolsRequestTaskAdapter::CurrentSymbolsRequestTaskAdapter()
{
    connect(task(), &CurrentSymbolsRequestTask::done, this, &TaskInterface::done);
}

void CurrentSymbolsRequestTaskAdapter::start()
{
    task()->start();
}

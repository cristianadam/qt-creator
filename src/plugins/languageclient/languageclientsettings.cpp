...
BaseSettings::BaseSettings()
{
    ...
    filePatterns.setSettingsKey("filePattern");
    filePatterns.setLabelText(Tr::tr("Language:"));
    filePatterns.setDisplayStyle(StringAspect::LineEditDisplay);
    filePatterns.setPlaceHolderText(Tr::tr("File pattern"));
    filePatterns.setToolTip(
    ...
    for (int behavior = 0; behavior < BaseSettings::LastSentinel ; ++behavior)
        startBehavior.addOption(startupBehaviorString(BaseSettings::StartBehavior(behavior)));
...
{
    return LanguageFilter{mimeTypes(), filePatterns().split(filterSeparator)};
}

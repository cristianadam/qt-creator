...
void EditorManagerPrivate::moveCurrentEditorToView(EditorView *targetView)
{
    ...
    // If the target split already shows this document, there is nothing to move;
    // just switch focus to it. Otherwise addEditor() would refuse the duplicate.
    if (targetView->editorForDocument(editor->document())) {
        activateView(targetView);
        return;
    }

    sourceView->removeEditor(editor, EditorView::RemoveTab);
    targetView->addEditor(editor);
    targetView->setCurrentEditor(editor);
    // Re-show the previously current editor (last in the MRU order) in the source split.
    const QList<IEditor *> remaining = sourceView->editors();
    if (!remaining.isEmpty())
        sourceView->setCurrentEditor(remaining.last());

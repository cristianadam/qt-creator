...
EditorManagerPrivate::EditorManagerPrivate(QObject *parent) :
    QObject(parent),
    m_revertToSavedAction(new QAction(Tr::tr("Revert to Saved"), this)),
    m_saveAction(new QAction(this)),
    m_saveAsAction(new QAction(this)),
    m_closeCurrentEditorAction(new QAction(Tr::tr("Close"), this)),
    m_closeAllEditorsAction(new QAction(Tr::tr("Close All"), this)),
    m_closeOtherDocumentsAction(new QAction(Tr::tr("Close Others"), this)),
    m_closeAllEditorsExceptVisibleAction(new QAction(Tr::tr("Close All Except Visible"), this)),
    m_gotoNextDocHistoryAction(new QAction(Tr::tr("Next Open Document in History"), this)),
    m_gotoPreviousDocHistoryAction(new QAction(Tr::tr("Previous Open Document in History"), this)),
    m_goBackAction(new QAction(Utils::Icons::PREV.icon(), Tr::tr("Go Back"), this)),
    m_goForwardAction(new QAction(Utils::Icons::NEXT.icon(), Tr::tr("Go Forward"), this)),
    m_gotoLastEditAction(new QAction(Tr::tr("Go to Last Edit"), this)),
    m_copyFilePathContextAction(new QAction(Tr::tr("Copy Full Path"), this)),
    m_copyLocationContextAction(new QAction(Tr::tr("Copy Path and Line Number"), this)),
    m_copyFileNameContextAction(new QAction(Tr::tr("Copy File Name"), this)),
    m_saveCurrentEditorContextAction(new QAction(Tr::tr("&Save"), this)),
    m_saveAsCurrentEditorContextAction(new QAction(Tr::tr("Save &As..."), this)),
    m_revertToSavedCurrentEditorContextAction(new QAction(Tr::tr("Revert to Saved"), this)),
    m_closeCurrentEditorContextAction(new QAction(Tr::tr("Close"), this)),
    m_closeAllEditorsContextAction(new QAction(Tr::tr("Close All"), this)),
    m_closeOtherDocumentsContextAction(new QAction(Tr::tr("Close Others"), this)),
    m_closeAllEditorsExceptVisibleContextAction(new QAction(Tr::tr("Close All Except Visible"), this)),
    ...
    m_filePropertiesAction(new QAction(Tr::tr("Properties..."), this)),
    m_pinAction(new QAction(Tr::tr("Pin"), this))
{
    d = this;
}
...
void EditorManagerPrivate::init()
{
    ...
    cmd->setDescription(Tr::tr("Revert File to Saved"));
    ...
    cmd->setDefaultKeySequence(QKeySequence(Tr::tr("Ctrl+W")));
    cmd->setAttribute(Command::CA_UpdateText);
    ...
    if (HostOsInfo::isWindowsHost()) {
        ...
        QAction *action = new QAction(Tr::tr("Alternative Close"), this);
        cmd = ActionManager::registerAction(action, Constants::CLOSE_ALTERNATIVE, editManagerContext);
        cmd->setDefaultKeySequence(QKeySequence(Tr::tr("Ctrl+F4")));
        cmd->setDescription(Tr::tr("Close"));
    ...
    cmd->setDefaultKeySequence(QKeySequence(Tr::tr("Ctrl+Shift+W")));
    mfile->addAction(cmd, Constants::G_FILE_CLOSE);
    ...
    cmd->setDefaultKeySequence(QKeySequence(useMacShortcuts ? Tr::tr("Alt+Tab") : Tr::tr("Ctrl+Tab")));
    ...
    cmd->setDefaultKeySequence(QKeySequence(useMacShortcuts ? Tr::tr("Alt+Shift+Tab") : Tr::tr("Ctrl+Shift+Tab")));
    mwindow->addAction(cmd, Constants::G_WINDOW_NAVIGATE);
    ...
    cmd->setDefaultKeySequence(QKeySequence(useMacShortcuts ? Tr::tr("Ctrl+Alt+Left") : Tr::tr("Alt+Left")));
    mwindow->addAction(cmd, Constants::G_WINDOW_NAVIGATE);
    ...
    cmd->setDefaultKeySequence(QKeySequence(useMacShortcuts ? Tr::tr("Ctrl+Alt+Right") : Tr::tr("Alt+Right")));
    mwindow->addAction(cmd, Constants::G_WINDOW_NAVIGATE);
    ...
    m_splitAction = new QAction(Utils::Icons::SPLIT_HORIZONTAL.icon(), Tr::tr("Split"), this);
    cmd = ActionManager::registerAction(m_splitAction, Constants::SPLIT, editManagerContext);
    cmd->setDefaultKeySequence(QKeySequence(useMacShortcuts ? Tr::tr("Meta+E,2") : Tr::tr("Ctrl+E,2")));
    ...
    advancedMenu->menu()->setTitle(Tr::tr("Ad&vanced"));
    ...
    globalMacroExpander()->registerFileVariables(kCurrentDocumentPrefix, Tr::tr("Current document"),
        [] {
    ...
    globalMacroExpander()->registerIntVariable(kCurrentDocumentXPos,
        Tr::tr("X-coordinate of the current editor's upper left corner, relative to screen."),
        []() -> int {
    ...
    globalMacroExpander()->registerIntVariable(kCurrentDocumentYPos,
        Tr::tr("Y-coordinate of the current editor's upper left corner, relative to screen."),
        []() -> int {
            IEditor *editor = EditorManager::currentEditor();
...
bool EditorManagerPrivate::skipOpeningBigTextFile(const FilePath &filePath)
{
    ...
    if (fileSizeInMB > d->m_settings.bigFileSizeLimitInMB
        ...
        const QString title = Tr::tr("Continue Opening Huge Text File?");
                                          ...
                                          EditorManager::OpenEditorFlags flags, bool *newEditor)
{
    ...
    if (factories.isEmpty()) {
        ...
        QMessageBox msgbox(QMessageBox::Critical, Tr::tr("File Error"),
                           Tr::tr("Could not open \"%1\": Cannot open files of type \"%2\".")
    ...
    while (factory) {
        ...
        if (factory->asEditorFactory()) {
            ...
            if (openResult == IDocument::OpenResult::ReadError) {
                QMessageBox msgbox(QMessageBox::Critical,
                                   Tr::tr("File Error"),
                                   Tr::tr("Could not open \"%1\" for reading. "
        ...
        if (errorString.isEmpty())
            errorString = Tr::tr("Could not open \"%1\": Unknown error.").arg(realFp.toUserOutput());
        ...
        if (ww) {
            ...
            if (promptVCS) {
                InfoBarEntry info(Id(kMakeWritableWarning),
                                  Tr::tr("<b>Warning:</b> This file was not opened in %1 yet.")
                                  .arg(versionControl->displayName()));
                info.addCustomButton(Tr::tr("Open"), &vcsOpenCurrentEditor);
                document->infoBar()->addInfo(info);
            } else {
                InfoBarEntry info(Id(kMakeWritableWarning),
                                  Tr::tr("<b>Warning:</b> You are changing a read-only file."));
                info.addCustomButton(Tr::tr("Make Writable"), &makeCurrentEditorWritable);
                document->infoBar()->addInfo(info);
            }
        } else {
                                            ...
                                            QAction *saveAsAction, QAction *revertToSavedAction)
{
    ...
    if (document && !document->displayName().isEmpty()) {
        ...
        saveAction->setText(Tr::tr("&Save %1").arg(quotedName));
        saveAsAction->setText(Tr::tr("Save %1 &As...").arg(quotedName));
        revertToSavedAction->setText(document->isModified()
                                     ? Tr::tr("Revert %1 to Saved").arg(quotedName)
                                     : Tr::tr("Reload %1").arg(quotedName));
    } else {
        saveAction->setText(Tr::tr("&Save"));
        saveAsAction->setText(Tr::tr("Save &As..."));
        revertToSavedAction->setText(Tr::tr("Revert to Saved"));
    }
}
...
void EditorManagerPrivate::updateActions()
{
    ...
    d->m_closeCurrentEditorAction->setText(Tr::tr("Close %1").arg(quotedName));
    d->m_closeAllEditorsAction->setEnabled(openedCount > 0);
    d->m_closeOtherDocumentsAction->setEnabled(openedCount > 1);
    d->m_closeOtherDocumentsAction->setText((openedCount > 1 ? Tr::tr("Close All Except %1").arg(quotedName) : Tr::tr("Close Others")));
...
void EditorManagerPrivate::vcsOpenCurrentEditor()
{
    ...
    if (!versionControl->vcsOpen(document->filePath())) {
        QMessageBox::warning(ICore::dialogParent(), Tr::tr("Cannot Open File"),
                            Tr:: tr("Cannot open the file for editing with VCS."));
    }
}
...
void EditorManagerPrivate::revertToSaved(IDocument *document)
{
    ...
    if (document->isModified()) {
        QMessageBox msgBox(QMessageBox::Question,
                           Tr::tr("Revert to Saved"),
                           Tr::tr("You will lose your current changes if you proceed reverting %1.")
                               .arg(QDir::toNativeSeparators(fileName)),
                           QMessageBox::Yes | QMessageBox::No,
                           ICore::dialogParent());
        msgBox.button(QMessageBox::Yes)->setText(Tr::tr("Proceed"));
        msgBox.button(QMessageBox::No)->setText(Tr::tr("Cancel"));

        QPushButton *diffButton = nullptr;
        auto diffService = DiffService::instance();
        if (diffService)
            diffButton = msgBox.addButton(Tr::tr("Cancel && &Diff"), QMessageBox::RejectRole);
    ...
    if (!document->reload(&errorString, IDocument::FlagReload, IDocument::TypeContents))
        QMessageBox::critical(ICore::dialogParent(), Tr::tr("File Error"), errorString);
}
                                                 ...
                                                 IEditor *editor)
{
    ...
    d->m_closeOtherDocumentsContextAction->setText(entry
                                                   ? Tr::tr("Close All Except \"%1\"").arg(quotedDisplayName)
                                                   : Tr::tr("Close Other Editors"));
...
void EditorManager::addPinEditorActions(QMenu *contextMenu, DocumentModel::Entry *entry)
{
    ...
    if (entry) {
        d->m_pinAction->setText(entry->pinned
                                ? Tr::tr("Unpin \"%1\"").arg(quotedDisplayName)
                                : Tr::tr("Pin \"%1\"").arg(quotedDisplayName));
    } else {
        d->m_pinAction->setText(Tr::tr("Pin Editor"));
    }
...
void EditorManager::addNativeDirAndOpenWithActions(QMenu *contextMenu, DocumentModel::Entry *entry)
{
    ...
    QMenu *openWith = contextMenu->addMenu(Tr::tr("Open With"));
...
bool EditorManager::openExternalEditor(const FilePath &filePath, Id editorId)
{
    ...
    if (!ok)
        QMessageBox::critical(ICore::dialogParent(), Tr::tr("Opening File"), errorMessage);
    return ok;
}

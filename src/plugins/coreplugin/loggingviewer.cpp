...
bool LoggingCategoryModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if (role == Qt::CheckStateRole && index.column() >= Column::Debug
        ...
        if (current != value.toBool()) {
            entry.setEnabled(msgType, value.toBool());
            return true;
        }
    } else if (role == Qt::DecorationRole && index.column() == Column::Color) {
...
public:
    ...
    {
        static LoggingViewManagerWidget instance(nullptr);
        return instance;
    }
...
private:
    ...
    QTableView *m_categoryView = nullptr;
...
{
    LoggingViewManagerWidget::instance().show();
    LoggingViewManagerWidget::instance().raise();
    LoggingViewManagerWidget::instance().activateWindow();
}

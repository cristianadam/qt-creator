                                                       ...
                                                       const QString &entry)
{
    ...
    if (!containsWildcard && !fullFilePath.exists() && directory.exists()) {
        ...
        filterEntry.acceptor = [fullFilePath] {
            QMetaObject::invokeMethod(EditorManager::instance(), [fullFilePath] {
                if (!fullFilePath.exists()) {
                    if (CheckableMessageBox::shouldAskAgain(ICore::settings(), kAlwaysCreate)) {
                        ...
                        messageBox.setText(Tr::tr("Create \"%1\"?")
                                               .arg(fullFilePath.shortNativePath()));
                    ...
                    QFile file(fullFilePath.toFSPathString());
                    file.open(QFile::WriteOnly);
                    file.close();
                    VcsManager::promptToAdd(fullFilePath.absolutePath(), {fullFilePath});
                }
                EditorManager::openEditor(fullFilePath);
            }, Qt::QueuedConnection);
            return AcceptResult();
        };

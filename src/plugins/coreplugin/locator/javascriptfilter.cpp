...
public:
    ...
    std::optional<JavaScriptOutput> output() const { return m_output; }
...
JavaScriptFilter::JavaScriptFilter()
    : m_javaScriptEngine(new JavaScriptEngine)
{
...
LocatorMatcherTasks JavaScriptFilter::matchers()
{
    ...
    const auto onDone = [storage](const JavaScriptRequest &request) {
        ...
        const std::optional<JavaScriptOutput> output = request.output();
        QTC_ASSERT(output, return);
        const QString result = output->m_output;
    ...
    const auto onError = [storage](const JavaScriptRequest &request) {
        ...
        QTC_ASSERT(output, return);
        LocatorFilterEntry entry;
        entry.displayName = output->m_output;
        storage->reportOutput({entry});
    };

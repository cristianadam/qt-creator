    ...
    void run(QPromise<LocatorMatcherTask::OutputData> &promise)
    {
        ...
        while (waitForData(&data)) {
            ...
            while (currentIndex < m_filterCount) {
                if (m_state == State::Canceled)
                    return;
                const auto &outputData = m_outputData.at(currentIndex);
                ...
                const auto &workingData = m_workingList.at(currentIndex);
                if (!workingData.has_value()) {
                    ...
                    m_workingList[currentIndex] = std::make_optional(WorkingData{dataForIndex, {}});
                    const LocatorMatcherTask::OutputData newData = mergeToCurrent
                        ? *outputData : m_workingList[mergeToIndex]->mergeWith(*outputData, m_state);
                    if (m_state == State::Canceled)
                        return;
                    ...
                    const LocatorMatcherTask::OutputData newData
                        = m_workingList[mergeToIndex]->mergeWith(workingData->m_data, m_state);
                    m_workingList[currentIndex] = std::make_optional<WorkingData>({});
                    if (m_state == State::Canceled)
                        return;
                    if (!hasGap && !newData.isEmpty())
                        promise.addResult(newData);
...
private:
    ...
    QList<std::optional<WorkingData>> m_workingList;
};
...
void OutputFilter::start()
{
    ...
    QTC_ASSERT(!isRunning(), return);
    QTC_ASSERT(m_filterCount, return);
...
void LocatorMatcher::start()
{
    ...
    const auto onFinish = [this](bool success) {
        return [this, success] { emit done(success); };
    };

...
{
    const auto kicker = [runControl, appInfo, tempFileStorage, startStopped](const SingleBarrier &barrier) {
        ...
        const auto onDone = [runControl, appInfo](const Process &process, DoneWith result) {
...
static Group deviceCtlPollingTask(RunControl *runControl, const Storage<AppInfo> &appInfo)
{
    ...
    const auto onPidSetup = [appInfo](Process &process) {
        process.setCommand({FilePath::fromString("/usr/bin/xcrun"),
                            {"devicectl",
                             "device",
                             "process",
    ...
    const auto onPidDone = [runControl, pidStorage](const Process &process, DoneWith result) {
    ...
    const auto onPollDone = [runControl, appInfo](const Process &process) {
        const Utils::expected_str<QJsonValue> resultValue = parseDevicectlResult(process.rawStdOut());
    ...
    return {
        pidStorage,
        ProcessTask(onPidSetup, onPidDone).withCancel(canceler()),
        Group {
            Forever {
    ...
    setProducer([](RunControl *runControl) -> RunWorker * {
        ...
        if (iosdevice && iosdevice->handler() == IosDevice::Handler::DeviceCtl) {
            if (IosDeviceManager::isDeviceCtlOutputSupported())
                return new RecipeRunner(runControl, deviceCtlRecipe(runControl, false));
...
static RunWorker *createWorker(RunControl *runControl)
{
    ...
    if (!isIosDeviceInstance /*== simulator */ || dev->handler() == IosDevice::Handler::IosTool) {
        ...
        runner = new RecipeRunner(runControl, deviceCtlRecipe(runControl, true));
    }

                             ...
                             const Storage<TemporaryFile> tempFileStorage, bool startStopped)
{
    ...
    const auto onDone = [runControl, appInfo, barrier = barrier->barrier()](DoneWith result) {
...
static Group iosKicker(const SingleBarrier &barrier, RunControl *runControl, const DebugInfo &debugInfo)
{
    ...
    const auto onIosSetup = [runControl, debugInfo, bundleDir, deviceType, device,
        ...
        runner.setStartHandler([runControl, debugInfo, bundleDir, device, barrier](IosToolHandler *handler) {
            ...
            QObject::connect(handler, &IosToolHandler::errorMsg,
                             runControl, [runControl](const QString &message) {
                QString res(message);
            ...
            QObject::connect(handler, &IosToolHandler::gotServerPorts, runControl,
                             [runControl, debugInfo, handler](Port gdbPort, Port qmlPort) {
                QUrl debugChannel;
                ...
                if (debugInfo.cppDebug) {
                    if (!gdbPort.isValid()) {
                        runControl->postMessage(Tr::tr("Failed to get a local debugger port."),
                                                ErrorMessageFormat);
                        handler->stop();
                        return;
                    }
                ...
                if (debugInfo.qmlDebugServices != NoQmlDebugServices) {
                    ...
                    runControl->postMessage(
                        Tr::tr("Listening for QML debugger on local port %1 (port %2 on the device).")
                            .arg(qmlPort.number()).arg(qmlPortOnDevice), LogMessageFormat);
                }
            });
            QObject::connect(handler, &IosToolHandler::gotInferiorPid, runControl,
                ...
                if (debugInfo.qmlDebugServices != NoQmlDebugServices && runControl->qmlChannel().port() == -1) {
                    runControl->postMessage(Tr::tr("Could not get necessary ports for the debugger connection."),
                                                                    ErrorMessageFormat);
                    handler->stop();
                    return;
                }
    ...
    const auto onIosDone = [runControl](DoneWith result) {
        if (result == DoneWith::Success)

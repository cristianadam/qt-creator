...
void XcodeProbe::setupDefaultToolchains(const QString &devPath)
{
    ...
    if (!clangProfile.cCompilerPath.isEmpty() || !clangProfile.cxxCompilerPath.isEmpty()) {
        for (const QString &arch : std::as_const(allArchitectures)) {
            // TODO The prefix "Apple Clang" is used by Qt Creator <= 17 to identify
            // the autogenerated iOS tool chains. Keep that until compatibility to that is no
            // longer relevant.
            const QString clangFullName
                = Tr::tr("Apple Clang iOS (%1 at \"%2\")").arg(arch, devPath);

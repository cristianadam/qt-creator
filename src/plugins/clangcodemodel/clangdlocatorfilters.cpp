...
#include <languageclient/currentsymbolsrequesttask.h>
                                 ...
                                 const QString &contents)
{
    ...
    const LocatorFilterEntries allMatches = LanguageClient::currentSymbols(storage.input(),
...
LocatorMatcherTask currentDocumentMatcher()
{
    ...
    const auto onFilterSetup = [=](AsyncTask<void> &async) {
        async.setFutureSynchronizer(ClangCodeModelPlugin::futureSynchronizer());

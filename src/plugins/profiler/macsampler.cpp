...
Result<task_t> waitForNewTask(pid_t pid)
{
    ...
    Result<task_t> task = ResultError(QString());
                                   ...
                                   std::atomic<int> *progressPercent)
{
    ...
    // sampler holds. Attach to the new one and start over: everything sampled so
    // far belongs to the image that is gone.
    for (;;) {

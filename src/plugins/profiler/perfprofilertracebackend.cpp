...
{
    return {&d->recordButton, &d->clearButton, &d->filterButton, &d->aggregateButton,
            &d->recordedLabel, &d->delayLabel, &d->tracePointsButton};
}

void PerfProfilerTraceBackend::setMenuActions(const QList<QAction *> &loadSaveActions,
                                              QAction *limitToRange, QAction *showFullRange)
{
    d->loadSaveActions = loadSaveActions;
    d->limitToRange = limitToRange;
    d->showFullRange = showFullRange;
}

...
void PerfProfilerTool::onWorkerCreation(RunControl *runControl)
{
    ...
    backend->setMenuActions({&d->m_loadPerfData, &d->m_loadTrace,
                             &d->m_saveTrace},
                            &d->m_limitToRange, &d->m_showFullRange);
    connect(backend, &PerfProfilerTraceBackend::recordingChanged,
            this, &PerfProfilerTool::recordingChanged);
...
void PerfProfilerTool::updateRunActions()
{
    ...
    d->m_saveTrace.setEnabled(current && !current->isEmpty());

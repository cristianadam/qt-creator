...
{
    const Utils::ProcessError error = m_process->error();
    QString message;
    if (error == Utils::FailedToStart) {
        message = Tr::tr("Failed to run trace point script: %1").arg(error);
    } else if ((m_process->exitStatus() == Utils::CrashExit) || (m_process->exitCode() != 0)) {
...
{
    if (m_process) {
        QTC_CHECK(m_process->state() == Utils::NotRunning);
        QDialog::accept();
    } else {
        runScript();
    }

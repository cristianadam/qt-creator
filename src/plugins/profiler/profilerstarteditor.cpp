...
public:
        ...
        , m_recordingPage(new RecordingPage)
    {
        ...
        connect(profilerRecorder(), &ProfilerRecorder::finished, this, [this](const FilePath &tracePath) {
            m_recordingPage->stop();
            setCurrentWidget(m_welcomePage);
            openTraceFile(tracePath);
        });
        connect(profilerRecorder(), &ProfilerRecorder::error, this, [this](const QString &message) {
            m_recordingPage->stop();
            setCurrentWidget(m_welcomePage);
            QMessageBox::warning(this, Tr::tr("Profiler"), message);
        });

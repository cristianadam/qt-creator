...
{
    return d->m_lockedProjectData.get<QList<ProjectInfo::ConstPtr>>(
        [](const CppModelManagerPrivate::SyncedProjectData &ld) {
            return Utils::transform<QList<ProjectInfo::ConstPtr>>(ld.m_projectData,
                                                                  [](const ProjectData &d) {
        ...
        if (it != ld.m_projectData.end() && it->projectInfo && it->fullyIndexed) {
            ...
            if (comparer.configurationOrFilesChanged()) {
                ...
                if (!removedFiles.isEmpty()) {
                    filesRemoved = true;
                    emit m_instance->aboutToRemoveFiles(
                        transform<QStringList>(removedFiles, &FilePath::toString));
...
QList<ProjectPart::ConstPtr> CppModelManager::projectPart(const FilePath &fileName)
{
    ...
    d->m_lockedProjectData.read([&fileName, &result](const auto &ld) {
        const auto it = ld.m_fileToProjectParts.constFind(fileName);
        if (it != ld.m_fileToProjectParts.constEnd())
            result = *it;
    });
    ...
    d->m_lockedProjectData.write(
        [&fileName, &result](auto &ld) {
            const FilePath canonicalPath = fileName.canonicalPath();
            const auto it = ld.m_fileToProjectParts.insert(fileName,
                                                           ld.m_fileToProjectParts.value(
                                                               canonicalPath));
            result = it.value();
        });
...
{
    d->m_lockedProjectData.write([](auto &ld) { ld.m_dirty = true; });
}
...
void CppModelManager::onAboutToRemoveProject(Project *project)
{
    ...
    d->m_lockedProjectData.write([project, &idsOfRemovedProjectParts](auto &ld) {
        ld.m_dirty = true;
        const QStringList projectPartsIdsBefore = ld.m_projectPartIdToProjectProjectPart.keys();

        ld.m_projectData.remove(project);
        CppModelManagerPrivate::recalculateProjectPartMappings(ld);

        const QStringList projectPartsIdsAfter = ld.m_projectPartIdToProjectProjectPart.keys();
...
void CppModelManager::onActiveProjectChanged(Project *project)
{
    ...
    const bool contains = d->m_lockedProjectData.get<bool>(
        [project](const auto &ld) { return ld.m_projectData.contains(project); });
...
{
    return d->m_lockedProjectData.update<FilePaths>([](auto &ld) {
        CppModelManagerPrivate::ensureUpdated(ld);
        return ld.m_projectFiles;
    });
}

HeaderPaths CppModelManager::headerPaths()
{
    return d->m_lockedProjectData.update<HeaderPaths>([](auto &ld) {
        CppModelManagerPrivate::ensureUpdated(ld);
        return ld.m_headerPaths;
    });
}

void CppModelManager::setHeaderPaths(const HeaderPaths &headerPaths)
{
    d->m_lockedProjectData.write([&headerPaths](auto &ld) { ld.m_headerPaths = headerPaths; });
}

Macros CppModelManager::definedMacros()
{
    return d->m_lockedProjectData.update<Macros>([](auto &ld) {
        CppModelManagerPrivate::ensureUpdated(ld);
        return ld.m_definedMacros;
    });
}

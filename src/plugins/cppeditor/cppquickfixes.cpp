...
public:
        ...
        , m_targetFilePath(targetFileName)
    {
        if (m_defpos == DefPosImplementationFile) {
            ...
            const FilePath resolved = declFile.parentDir().resolvePath(
                           m_loc.isValid() ? m_loc.filePath() : m_targetFilePath);
            setPriority(2);
...
public:
        ...
        , m_headerFilePath(funcDef->symbol->filePath())
    {
        if (m_type == MoveFuncDefRefactoringHelper::MoveOutside) {
            ...
            const FilePath resolved = m_headerFilePath.parentDir().resolvePath(m_cppFilePath);
            setDescription(QCoreApplication::translate("CppEditor::QuickFix",
...
public:
        ...
        , m_toRange(toRange)
    {
        if (m_toFilePath == m_fromFilePath) {
            ...
            const FilePath resolved = m_fromFilePath.parentDir().resolvePath(m_toFilePath);
            setDescription(QCoreApplication::translate("CppEditor::QuickFix",
                                                       "Move Definition to %1")
                           .arg(resolved.displayName()));
        }

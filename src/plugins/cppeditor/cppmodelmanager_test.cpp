...
void ModelManagerTest::testRefreshTimeStampModifiedIfSourcefilesChange()
{
    ...
    QFETCH(QStringList, initialProjectFiles_);
    QFETCH(QStringList, finalProjectFiles_);

    const FilePaths initialProjectFiles = transform(initialProjectFiles_, &FilePath::fromString);
    const FilePaths finalProjectFiles = transform(finalProjectFiles_, &FilePath::fromString);

    TemporaryCopiedDir temporaryDir(MyTestDataDir(QLatin1String("testdata_refresh2")).path());
    const FilePath fileToChange = temporaryDir.absolutePath(fileToChange_);
    const FilePaths initialProjectFilePaths = toAbsolutePaths(initialProjectFiles_, temporaryDir);
    const FilePaths finalProjectFilePaths = toAbsolutePaths(finalProjectFiles_, temporaryDir);
    ...
    RawProjectPart rpp;
    rpp.setQtVersion(Utils::QtMajorVersion::Qt5);
    auto files = Utils::transform<ProjectFiles>(initialProjectFiles, [](const FilePath &f) {
    ...
    files = Utils::transform<ProjectFiles>(finalProjectFiles, [](const FilePath &f) {
        return ProjectFile(f, ProjectFile::CXXSource);
    });

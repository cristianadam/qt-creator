...
void registerMcpTools()
{
    ...
    ToolRegistry::registerTool(
        Tool{}
            ...
            .description(
                "Searches the whole project's C++ symbols (classes, functions, enums, "
                "declarations) by name, using Qt Creator's code model index - the "
                "project-wide \"go to symbol\". Give a case-insensitive name substring; "
                "optionally restrict by \"kind\" and cap the result count with \"limit\". "
                "Results are ranked (exact, then prefix, then substring name matches) so "
                "the most relevant survive the cap; \"total_matches\" and \"truncated\" "
                "report when the cap dropped matches. Returns each match with its name, "
                "kind, fully qualified scope, type/signature, and file with 1-based line "
                "and column.")
            .annotations(ToolAnnotations{}.readOnlyHint(true))
            .inputSchema(
                Tool::InputSchema{}
                    ...
                    .addProperty(
                        ...
                        QJsonObject{
                            ...
                            {"description",
                             "Optional: restrict to one kind of symbol."}})
                    .addProperty(
                        "limit",
                        QJsonObject{
                            {"type", "integer"},

...
{
    Parameters parameters = search->userData().value<Parameters>();
    ...
    const auto onSetup = [search, parameters, projectFileNames](Async<SearchResultItem> &task) {
        task.setConcurrentCallData(&Internal::search, CppModelManager::snapshot(),
...
{
    QStringList types;
    if (m_symbolsToSearch & Classes)
        types.append(Tr::tr("Classes"));
    if (m_symbolsToSearch & Functions)
        types.append(Tr::tr("Functions"));
    if (m_symbolsToSearch & Enums)
        types.append(Tr::tr("Enums"));
    if (m_symbolsToSearch & Declarations)
...
void SymbolsFindFilterConfigWidget::getState()
{
    ...
    m_typeClasses->setChecked(symbols & Classes);
    m_typeMethods->setChecked(symbols & Functions);
    m_typeEnums->setChecked(symbols & Enums);
    m_typeDeclarations->setChecked(symbols & Declarations);

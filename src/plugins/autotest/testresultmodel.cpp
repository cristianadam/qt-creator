...
TestResultItem *TestResultItem::findParentItemFor(const TestResultItem *item,
...
void TestResultItem::updateParent(const TestResultItem *item)
{
    ...
    if (!changed && !changedType)
        return;
    if (model())
        emit model()->dataChanged(parentItem->index(), parentItem->index());

...
static bool equalIgnoringWhitespace(const QString &left, const QString &right)
{
    ...
    while (l < left.size() && r < right.size()) {
        if (left.at(l).isSpace()) {
            ++l;
        } else if (right.at(r).isSpace()) {
            ++r;
    ...
    while (l < left.size() && left.at(l).isSpace())
        ++l;
    while (r < right.size() && right.at(r).isSpace())
        ++r;
...
static void trimWhitespaceHighlights(TextLineData &line)
{
    ...
    for (auto it = line.changedPositions.cbegin(); it != line.changedPositions.cend(); ++it) {
        ...
        while (start < end && line.text.at(start).isSpace())
            ++start;
        while (end > start && line.text.at(end - 1).isSpace())
            --end;
        if (end > start)

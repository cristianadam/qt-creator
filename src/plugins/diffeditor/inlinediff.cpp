...
public:
    ...
    void update(const QList<InlineDiffChunk> &hunks)
    {
        ...
        Utils::sort(sorted, [](const InlineDiffChunk &a, const InlineDiffChunk &b) {
            return a.editorStartLine < b.editorStartLine;
        });
        ...
        for (const InlineDiffChunk &hunk : std::as_const(sorted)) {
            // 1:1 equal region up to the hunk
...
private:
    ...
    void applyDecorations()
    {
        ...
        if (m_viewMode == InlineDiffViewMode::Inline) {
            ...
            // aligner; the decorators only add the change highlights and the
            // fixed rows reserved for the conflict resolution buttons
            m_decorator->apply({}, m_model.changes, conflictSpacers);
            if (m_baselineDecorator) {
                // compensate the conflict button rows on the baseline side to
                // keep both sides the same height
                QList<InlineDiffDecorator::Spacer> baselineSpacers;
                for (const ConflictRegion &conflict : std::as_const(m_conflicts)) {

...
// Reapply all required properties after setting new text document
void UnifiedDiffEditorWidget::reapplyTextDocumentSettings()
{
    setReadOnly(true);

    // HACK: in order to apply the right font and zoom
    ...
    connect(m_watcher.get(), &QFutureWatcherBase::finished, this, [this] {
        if (m_watcher->isCanceled()) {
            ...
            TextDocumentPtr doc(result.textDocument);
            {
                ...
                result.textDocument->moveToThread(thread());
                setTextDocument(doc);

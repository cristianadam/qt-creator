...
- Two non-QML `TimelineModel` subclasses already exist: `CtfTimelineModel`
  ...
  *lives inside the qmlprofiler plugin* -- precedent for an umbrella plugin.
...
## 6. The `qtprofiler` plugin merge

Fold `perfprofiler` into `qmlprofiler` (rename `qtprofiler`).
...
- **Precedent:** the CTF visualizer already lives inside `qmlprofiler`.
- **V4 glue wants to live here:** the symbolication contract is QML-engine-aware,
  so it belongs next to the QML profiler code.
...
3. **M3 -- `qtprofiler` plugin merge:** one tool/perspective/zoom/clock.

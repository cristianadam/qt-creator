...
static QStringList possibleQMakeCommands(const OsType osType)
{
    ...
    if (osType == OsTypeWindows)
        return QStringList{"qmake.exe", "qtpaths.exe"};
...
static bool isQmake(FilePath path)
{
    if (!path.isExecutableFile())
        return false;
    if (isQtChooser(path))
        path = qtChooserToQmakePath(path.symLinkTarget());
    if (!path.exists())
        return false;
    return !qtVersionForQMake(path).isEmpty();
}
...
FilePaths findQtsInEnvironment(const Environment &env, const FilePath &deviceRoot)
{
...
FilePaths findQtsInPaths(const Utils::FilePaths &paths)
{
    FilePaths qmakeList;
...
bool isQtChooser(const FilePath &filePath)
{
...
FilePath qtChooserToQmakePath(const Utils::FilePath &qtChooser)
{
...
QString filterForQmakeFileDialog(const OsType osType)
{

...
QtVersionManagerSignals *QtVersionManagerSignals::instance()
{
    static QtVersionManagerSignals theSignals;
    return &theSignals;
}
...
void QtVersionManagerImpl::triggerQtVersionRestore()
{
    ...
    emit QtVersionManagerSignals::instance()->qtVersionsLoaded();

    emit QtVersionManagerSignals::instance()->qtVersionsChanged(
...
void QtVersionManagerImpl::updateFromInstaller(bool emitSignal)
{
    ...
    if (emitSignal)
        emit QtVersionManagerSignals::instance()->qtVersionsChanged(added, removed, changed);
}
...
void QtVersionManager::addVersion(QtVersion *version)
{
    ...
    emit QtVersionManagerSignals::instance()->qtVersionsChanged(QList<int>() << uniqueId, QList<int>(), QList<int>());
...
void QtVersionManager::removeVersion(QtVersion *version)
{
    ...
    emit QtVersionManagerSignals::instance()->qtVersionsChanged(QList<int>(), QList<int>() << version->uniqueId(), QList<int>());
    qtVersionManagerImpl().saveQtVersions();
    delete version;
}
...
void QtVersionManagerImpl::setNewQtVersions(const QtVersions &newVersions)
{
    ...
    if (!changedVersions.isEmpty() || !addedVersions.isEmpty() || !removedVersions.isEmpty())
        emit QtVersionManagerSignals::instance()->qtVersionsChanged(addedIds, removedIds, changedIds);
}

...
QStringList RecordWidget::ffmpegParameters(const ClipInfo &clipInfo) const
{
    ...
    const bool captureCursor = Internal::settings().captureCursor();
    const QString captureCursorStr = captureCursor ? "1" : "0";
    ...
    case OsTypeWindows: {
        ...
        filter.append(":draw_mouse=" + QLatin1String(captureCursor ? "true" : "false"));

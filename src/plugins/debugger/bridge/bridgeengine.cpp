...
void BridgeEngine::insertBreakpoint(const Breakpoint &bp)
{
    ...
    // Correlation back is by the stable modelid, not file:line. See
    // bridge-protocol-design.md (5.4).
...
void BridgeEngine::doUpdateLocals(const UpdateParameters &params)
{
    ...
    // (the 'expanded' set among them). See bridge-protocol-design.md.
...
void BridgeEngine::handleFetchVariablesResponse(const QJsonObject &response)
{
    ...
    // the existing GdbMi reader and feed the shared updateLocalsView(). See
    // bridge-protocol-design.md (native variable plane).

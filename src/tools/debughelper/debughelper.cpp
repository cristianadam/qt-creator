...
Q_LOGGING_CATEGORY(log, "qtc.debughelper", QtWarningMsg);
...
{
    qCInfo(log) << "Inferior started ( pid:" << inferiorProcess.processId() << ")";
#ifdef Q_OS_WIN
    sendThreadId(win_process_information->dwThreadId);
    sendPid(inferiorProcess.processId());
#else
...
void setupUnixInferior()
{
    ...
    if (debugMode) {
        ...
        // We need to cheat a little here. When interrupting the process in the childProcessModifier
        // QProcess will not emit "started". To get the child pid, we manually poll for it from
        // a piece of shared memory (shared_child_pid).
        ...
        QObject::connect(&pollPidTimer, &QTimer::timeout, [&] {
            if (*shared_child_pid) {
                qCInfo(log) << "Received pid during polling:" << *shared_child_pid;
                sendPid(*shared_child_pid);
                pollPidTimer.stop();
            } else {
...
void startProcess(const QString &executable, const QStringList &arguments, const QString &workingDir)
{
    ...
    QObject::connect(&inferiorProcess, &QProcess::finished, &onInferiorFinished);
    QObject::connect(&inferiorProcess, &QProcess::errorOccurred, &onInferiorErrorOccurered);
...
void forwardSignal(int signum)
{
    ...
    kill(inferiorProcess.processId(), signum);
}
#else
...
std::optional<int> tryParseCommandLine(QCoreApplication &app)
{
    ...
    if (commandLineParser.isSet("verbose"))
        QLoggingCategory::setFilterRules("qtc.debughelper=true");
...
void resumeInferior()
{
    qCDebug(log) << "Continuing inferior... (" << inferiorProcess.processId() << ")";
#ifdef Q_OS_WIN
    ResumeThread(win_process_information->hThread);
#else
    kill(inferiorProcess.processId(), SIGCONT);
#endif
}
...
void onControlSocketReadyRead()
{
    ...
    for (auto ch : data) {
        ...
        case 'i': {
            ...
            kill(inferiorProcess.processId(), SIGINT);
            break;
        }
#endif
        case 'c': {
